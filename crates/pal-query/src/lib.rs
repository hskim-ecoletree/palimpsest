...
//! # 여기 없는 것
//!
//! **파일 경계를 넘는 해소가 없다**(옛 F05 §0). `symbol.reaches` 가 걷는 엣지는 파일 **안**
...
pub enum QueryResult {
    ...
    ///
    /// [옛 F12 §2] 가 못 박았다 — *"좌표로 해소되지 않은 계획 항목을 「계획대로」나
...
pub struct QueryCtx<'a> {
    ...
    /// ★ 좌표 하나로 결박을 **색인으로** 찾는 자리 (옛 F11 §3.1).
    ///
    /// # 왜 [`Self::bindings`] 를 걸러 쓰지 않는가
    ///
    /// [옛 F11 §3.1] 이 못 박았다 — *"핵심은 `BOUND_BY` 역방향 색인이다. 정방향만 있으면
...
/// 점진 회상 — **요약 + 상위 N, 그리고 낡은 것은 상한을 안 탄다** (옛 F11 §3.3).
///
