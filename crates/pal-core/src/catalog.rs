...
//! 다른 쪽을 끄고, 하필 **통제가 필요한 표본에서만** 멎는다(F05 의 오라클이 그렇게
//! 꺼졌다 · `[f06.1.pass]` ①).
//!
//! # 런타임에 안 읽힌다
//!
...
// 카탈로그의 사본이 아니라 **코드가 스스로 아는 것**이다 — 표면이 `--list` 를 산출할 때
...
impl QueryName {
    ...
    pub const fn summary(self) -> &'static str {
        match self {
            ...
            Self::SymbolReaches => "이 심볼에서 닿는 것들 — **예산 생략이 있는 BFS**",
            Self::GraphDump => "노드와 엣지 전부 — 바깥 대조 도구가 읽는 창",
            Self::BindingStatus => "결박마다 상태 + **반경** + 무엇이 켰는가",
            Self::NarrativeUnbound => "좌표를 못 찾은 문서 조각들 — **사람의 작업 목록**",
            Self::BindingTouch => "좌표 하나를 만진다 — **걸린 것**과 ★ **지켜보는 것**을 함께 싣는다",
