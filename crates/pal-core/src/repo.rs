...
/// 저장소 루트 기준 상대 경로. **구분자는 항상 `/` 다** — git 이 그렇게 잰다.
///
...
/// 머클 계산과 git 인덱스 캐시가 거기 있다([옛 F01 §3.2](../../../docs/plan/disposal-map.md)).
...
impl TreeRef {
    ...
    /// 워킹트리와 일치하는가. 표면이 이 문장을 그대로 출력한다.
    #[must_use]
...
///
/// [옛 DESIGN §1.1](../../../docs/plan/disposal-map.md) 은 `Snapshot` 을 `{(repo_id, TreeRef)}` 로
...
mod tests {
    ...
    /// [`RepoPath::new`] 로는 성립하지 않는다.** Windows 의 `Path` 는 `\` 를 내고, 그 값이
