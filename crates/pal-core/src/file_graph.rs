...
//!
//! # 없는 자리와 있는 자리 — **비어 있는 것이 아니라 능력이 없는 것이다**
//!
//! [`FileGraph::scopes`] 와 [`FileGraph::export_digest`] 는 **이제 필드다** — #48 이
//! TypeScript 에서, #130 이 Rust 에서 세웠다. 둘 다 [`Capable`] 이고, 그것이 요점이다:
//! **빈 값으로 세우지 않는다.** 빈 스코프 목록은 *"스코프가 없는 파일"* 과 *"이 빌드가
//! 스코프를 안 만든다"*(Kotlin)를 같은 출력으로 만들고, 그것이 [`crate::Capable`] 이
//! 존재하는 이유의 정면 위반이다.
//!
//! 아직 필드가 **아닌** 것: `local_refs`·`raw_refs`. 파일 간 참조 해소는 **F07** 이다.
//!
//! # 왜 `Deserialize` 가 없는가
//!
...
pub struct ImportedItem {
    ...
    pub local: String,
    /// 이 `use` 를 감싸는 **인라인 `mod` 의 겹 수**. 파일 최상위면 0.
    ///
    /// # 왜 필요한가 (2026-09-10 · 독립 리뷰 라운드 2)
    ///
    /// `super::` 는 **쓴 자리의 모듈**을 기준으로 한 칸 올라간다. 그런데 2 층은 파일
    /// 경로로만 모듈 자리를 계산하므로, 인라인 `mod` 안의 `use super::{…}` 를
    /// **파일의 부모 모듈**로 보낸다 — 한 칸 높다.
    ///
    /// 실측: `crates/pal-cli/src/install/manifest.rs` 의 `mod tests` 안
    /// `use super::{… Rel …}` 가 `crates/pal-cli/src/install.rs` 로 풀렸고, 꼬리 `new`
    /// 가 그 파일의 **사적** `Report::new` 를 잡아 **거짓 엣지 3 건**이 섰다.
    /// 소유자가 2026-09-08 에 `super::` 접두를 축1 **안**으로 들였으므로 그 자리는
    /// 이 회차가 지는 자리다.
    ///
    /// **이 값이 있으면 `super` 의 첫 겹들이 파일 안에서 소진된다** — 인라인 겹만큼은
    /// 올라가지 않고 그 파일의 모듈에 머문다.
