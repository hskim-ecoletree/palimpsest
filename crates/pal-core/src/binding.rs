...
/// 뒤집힘도 **사실이 아닌 것을 사실로** 내는 것이다(반대 방향).
///
...
///
/// # 밖에서는 [`Binding::new`] 로만 만든다 ([옛 F03 §3.3])
///
...
pub struct Binding {
    pub id: BindingId,
    /// **무엇이** 걸렸나 — 결정·계획·라벨 같은 비코드 개체([`crate::EntityId`] · 옛 F09 §4.3).
    ///
...
/// 이 결박이 **어떻게 섰는가** — 그리고 **길이 둘뿐이다** (옛 F10 §3.3 · §1).
///
/// # `Option` 이 아니다
///
...
impl Binding {
    ...
    ///
    /// [`crate::SymbolIdentity`] 를 받지 않는 이유가 옛 F03 §3.3 이다: `Unavailable` 에는
    ...
    /// 제안 하나를 **승인해서 새 결박을 낳는다** (옛 F10 §3.3 · §4).
    ///
...
/// portal-backend 1,340 심볼이 전부 여기 걸린다. 그것이 옛 DESIGN §15-42 가 경고한
...
pub enum UndeterminableReason {
    ...
    /// `Unavailable` 에서는 그 값을 꺼낼 수 없다(옛 F03 §3.3). 그러므로 이 사유는
...
///     [`Binding`] 이 `SymbolId` 를 요구하고 그것이 옛 F03 §3.3 이 타입으로 세운 것이다.
...
pub enum CodeFreshness {
    ...
    /// 사람의 판단을 부른다(옛 F09 §2). 뭉개면 *"고치면 되는 것"* 과 *"결정을 다시 해야
    /// 하는 것"* 이 같은 화면이 된다.
    ///
...
mod tests {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 승격 — **세탁 금지가 타입인지 시험한다** (옛 F10 §1 · §3.3)
    // ─────────────────────────────────────────────────────────────────────────
...
/// # 왜 판정마다가 아니라 답마다인가
///
/// 감지기는 **한 답 전체가 같은 것을 쓴다.** 결박마다 실으면 같은 값이 N 번 복제되고,
/// 그 복제본들이 갈릴 자리가 생긴다.
///
...
pub struct BindingReport {
    ...
    ///
    /// 0 이 아니면 그만큼은 속성·수신자 변경에 `stale` 이 안 켜진다 — 옛 판 결박이다.
