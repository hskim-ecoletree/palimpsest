...
//!
//! 옛 백서 P2 는 *사실과 추론* 2분할을 요구했고 그것은 가드 라벨을 담지 못한다(추출되지
...
pub enum ResolutionGrade {
    /// 임포트·정의가 해소되어 대상이 유일.
    Exact,
    /// 스코프 해소로 후보가 유일 — **`FileGraph::scopes` 가 선 언어에서만.**
    ///
    /// ⚠ **자격을 지는 것은 언어 등급 글자가 아니라 스코프 체인의 존재다.**
    /// 2026-09-08 까지 이 줄은 *"L2 이상에서만"* 이었고 그것이 거짓이 됐다 — Rust 는
    /// `grade_of` 가 `L1` 인 채로 스코프 체인을 세워 이 등급의 엣지를 산출한다(#130 ·
    /// 회차 `2026-09-07-rust-scope-references`). 등급 글자는 **`body_digest` 가 지역
    /// 이름을 지우는가**를 진다(`IdentityGrade`). 두 축이 다르다.
    ///
    /// 소비 규칙은 안 바뀐다 — *"컨설팅(정밀도 우선)은 `Exact|Scoped` 만 쓴다"* 는
    /// **후보가 유일한가**를 묻는 것이고, 그 사실은 여전히 참이다.
    Scoped,
