//! 예산을 들고 다니는 넓이 우선 탐색 — **옛 F05 §5.2. 이 실행기가 존재하는 이유다.**
//!
...
//! 저장 기술에 의존하지 않는다(stack §4.1). 그리고 그 형태 덕에 **이 생략 규칙이
...
//! 훨씬 많이 안 본 것이다 — 그것이 이 제품이 고발하는 조용한 생략이다.
...
mod tests {
    ...
    #[test]
    fn 생략은_쌓인다() {
        // 여러 질의가 한 응답 묶음에 실리면 생략이 합산되어야 한다.
        let mut el = Elision::none();
        let b = Budget::new(CANDIDATE_LIMIT, PROVISIONAL_PATH_PRODUCT_MAX, 1, 10_000);
