...
pub struct SymbolNode {
    pub id: SymbolId,
    pub path: RepoPath,
    /// 파일 → 클래스 → (중첩)클래스. **[`id`] 의 성분이다**(옛 F03 §3.2).
    ///
    /// # 왜 유도하지 않고 싣는가
    ///
...
/// 이 좌표에 걸린 것.
///
/// 옛 F11 §2 는 일곱 종류를 적었다 — 결정 · 대체 이력 · 라벨 · 계획 항목 · 결함 계보 ·
...
pub enum BoundItem {
    ...
    Note {
        ...
        ///
        /// 0 이 아니면 그만큼은 속성·수신자 변경에 `stale` 이 안 켜진다. 화면이 그것을 말한다.
        decor_unwatched: usize,
        /// 언제 걸었나 — **표시용이다. 앵커가 아니다**(옛 F09 §6).
        ///
        /// 낡음 판정은 이 값을 안 읽는다. **정렬은 읽는다** — 정렬은 화면의 일이고
        /// [옛 F11 §3.3] 이 *"결박 시점이 최근인 것 우선"* 을 사실 기반 정렬로 요구했다.
...
///
/// [옛 F11 §5] 가 정렬에서 관련도 점수를 기각했다: *"점수는 근거가 없다."*
...
pub enum NearKind {
    ...
    /// `resolveClaimBranch`.
    Spelling,
    /// 한쪽이 다른 쪽의 **부분 문자열**이다(대소문자 무시) — [옛 F11 §4] 의 *"부분 매칭"*.
    Substring,
}
...
/// 결박을 화면에 세울 순서 — **사실 기반 정렬이다. 점수가 아니다**([옛 F11 §3.3]).
///
/// ```text
...
/// **낡은 것은 상한에 걸려도 실린다** ([옛 F11 §3.3] · `[f11.pass]` ④).
///
...
/// `touch` 하나의 답.
///
/// **일곱 자리를 한 번에 반환하는 것이 요점이다**(옛 F11 §2) — 사람은 *"이 코드에 관련된 게
