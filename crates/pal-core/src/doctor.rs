...
///
/// **자리표시다.** §12.4 의 예산 표에 `doctor` 의 표본 크기가 없다. 값을 정하는 것은
/// F05 의 예산 회귀이고, 여기서는 *"기본이 표본이고 전수가 명시적 호출"* 이라는 §12.7 의
/// 분기가 **존재하는가**까지만 선다(`[f22.4].does_not_prove.not_full_scan_cost`).
