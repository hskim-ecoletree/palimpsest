...
//! 캐시 키의 성분으로 요구하는 자리에 있다. 둘을 한 곳에 모으면 *"이 값을 바꾸면
...
///
/// 강등되는 27 중 **26 이 선언을 하나도 못 만든다.** 그것들이 `partial`(*"일부는 읽었다"*)로
...
/// 이 값을 정한 회차가 잰 남의 TS 저장소의 가장 긴 사슬은 **1** 겹이었다.
...
/// 탐색 깊이 상한 — 3 홉. **후보 경유는 1 홉으로 잰다**(옛 F05 §5.2).
///
...
/// 쓴다"* 고 이름으로 말한다. **그 부재를 `cargo xtask check` 가 잰다**
...
/// `touch` 화면이 싣는 **호출자 자리**의 수 — 나머지는 수와 펴는 질의로만 싣는다.
///
/// # 왜 이것이 예산인가
///
/// *"얼마까지 보고 멈추는가"* 다. 호출자가 수백인 심볼에서 자리를 전부 실으면 답 한 장이 그
/// 목록에 먹힌다([R-11](../../../docs/plan/00-risks.md#r-11)). 싣지 않은 몫은 버려지지 않는다 —
/// `symbol.callers` 가 전부 돌려주고, 화면이 그 명령을 그대로 적는다.
///
/// # 자리표시다 — **어느 측정도 이 값을 정하지 않았다**
///
/// 잠긴 의도가 *"앞 몇 개"* 로만 적었다. 한 화면에서 눈으로 따라갈 수 있는 줄 수로 골랐고,
/// 확정은 실사용 기록이 한다.
pub const PROVISIONAL_TOUCH_CALLER_PLACE_MAX: usize = 5;
