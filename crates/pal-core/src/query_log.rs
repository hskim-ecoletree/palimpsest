//! 질의 로그 — **F05 부터 쓴다.**
//!
//! 옛 F05 §5.3: *"모든 질의 실행이 접근한 좌표를 `QUERY_LOG` 에 남긴다. F17(커버리지 계산)이
//! 나중에 새 장치를 만들지 않고 **F05 부터 쌓인 로그를 읽게** 하기 위해서다.
//! **처음부터 켜지 않으면 그 기능은 데이터가 없어 착수할 수 없다.**"*
//!
//! # 이것은 파생이 아니다 — 그런데 2층에 있다
//!
...
pub enum QueryName {
    ...
    /// 대조가 멎은 채로 돌았다.
    ///
    /// **그리고 이름이 여기 서야 질의 로그가 남는다.** 옛 판의 `pal touch` 는
    ...
    /// **기준선이 그 문서 안에 있기 때문이다**([`crate::PlanBaseline`]) — [옛 F12 §4] 가
...
impl QueryName {
    ...
    /// 이 빌드가 답하는 질의 전부. **표면이 이것을 그대로 출력한다.**
...
/// 질의 하나가 남기는 줄 — 옛 F05 §5.3 의 일곱 성분.
///
...
pub struct QueryLogEntry {
    ...
    /// 이 답이 **실제로 만진** 좌표. F17 의 커버리지가 이것을 잰다.
