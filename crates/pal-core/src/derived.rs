...
//!
//! [옛 DESIGN §3.1](../../../docs/plan/disposal-map.md)이 *"출처 필드는 불변이고 승격은 새 노드 생성"*
...
//! | 성분 | 다르면 | 왜 |
//! |---|---|---|
//! | 출처 · 생산자 | **두 노드** | 두 엔진의 같은 판정이 나란히 성립한다. **붕괴한 것은 어긋날 수 없다** |
//! | 재현 입력 | **두 노드** | 다른 시점·다른 범위의 산출이 덮어쓰지 않고 쌓인다 |
//! | **본문** | **한 노드** | 같은 근거로 다시 만든 요약이 문장만 달라졌다고 새 노드가 되면 **계보가 소음이 된다** |
//!
//! 앞의 셋만 보면 *"전부 다른 값을 산출하는"* 해시가 만점을 받고, 마지막 하나만 보면
...
/// 재현 입력 — **`extracted` 사실은 자기 재현 입력을 실어야 한다** ([옛 DESIGN §3](../../../docs/plan/disposal-map.md)).
///
/// # 커밋 하나가 아니다 (U14-c)
///
...
/// 의존하지 않고 같은 저장소에서 언제나 같은 답을 돌려준다. `observed` 로 배정하면
...
/// 파생 노드의 정체성.
///
/// **[`SymbolId`] 와 다른 네임스페이스에 있다** — 접두어가 그 일을 한다. 없으면 파생
...
impl DerivedId {
    ...
    /// 무엇에 대한 것인가, 나머지 셋이 [옛 DESIGN §1.2 D33](../../../docs/plan/disposal-map.md)이
    /// 못 박은 성분이다.
    ///
    /// # 길이를 앞에 붙이는 이유
    ///
    ...
    pub fn compute(
        ...
        // `provider(sast-b)` 의 판정이 한 노드로 뭉개지면 엔진 간 불일치가 성립하지 않는다.
...
mod tests {
    ...
    #[test]
    fn 같은_대상의_판정_셋이_세_노드로_성립한다() {
        // **이것이 이 게이트의 정수다.** 하나로 뭉개지면 last-writer-wins 로 생산자
    ...
    fn 같은_종류_다른_엔진도_다른_노드다() {
        // `provider(sast-a)` 와 `provider(sast-b)` — 이름만 성분이면 여기서 뭉개진다.
        // 뭉개지면 **엔진 간 불일치**(옛 F16 §1.2 의 다섯째 몫)가 성립하지 않는다.
    ...
    fn 재현_입력만_달라도_다른_노드다() {
        // SAST 의 다중 taint 흐름을 가르는 것도 여기다 — 옛 F16 §4.2-3.
    ...
    fn 본문만_다른_둘은_한_노드다() {
        ...
        // *"전부 다른 값을 산출하는"* 해시가 만점을 받는다. 그러면 같은 근거로 다시 만든
        // 요약이 문장만 달라져도 새 노드가 되고 **계보가 소음이 된다**(옛 DESIGN §1.2).
        let mut 그래프: BTreeMap<DerivedId, &str> = BTreeMap::new();
        let id1 = 에이전트();
        ...
        // **갱신이다.** id 가 같으면 갱신, 다르면 새 노드(옛 DESIGN §1.2 의 병합 규칙).
    ...
    fn 네임스페이스가_코드_좌표와의_충돌을_막는다() {
        // 접두어가 하는 일의 전부를 그대로 보인다 — 없으면 같은 성분이 같은 값을 산출한다.
