...
//!
//! F09 §4.3 바로 아래 문장이 *"노드·엣지 타입을 닫힌 열거로 하드코딩하지 않는다"* 인데
...
//!
//! F09 §4.3 이 `id: Ulid` 라고 이름으로 적었다. `ulid` 크레이트(3.0)는 **열다섯 개를
...
impl EntityRegistry {
    ...
    ///
    /// F09 §4.3 이 요구한 것이 정확히 이것이다: *"미등록 종류를 「알 수 없는 종류의
    /// 결박 N건」으로 **보이게** 처리한다."*
    #[must_use]
...
mod tests {
    ...
    fn 미등록_종류를_조용히_버리지_않는다() {
        // **★ F09 §4.3 이 요구한 자리다.** 모르는 종류도 답이 있다.
