...
//! 옛 백서 §6.3 의 *"하한임이 표시되어야 한다"* 가 문장이 아니라 데이터가 되는 지점이다.
...
/// 싣고 그것이 **이 빌드에 박힌 상수**이기 때문이다. **생략에는 그 문제가 없다** —
...
pub enum ElisionReason {
    ...
    ///
    /// [`crate::Budget`] 의 넷은 **탐색** 예산이고(후보·경로 곱·깊이·노드), 이 생략은
...
// ─────────────────────────────────────────────────────────────────────────────
// 이관(fold) — **부피를 옮긴 것. 생략이 아니다** (옛 F06 §4.3 · `[f06].fold_is_not_elision`)
// ─────────────────────────────────────────────────────────────────────────────

/// 응답 묶음에서 **요약만 싣고 본체를 다른 질의로 옮긴** 자리.
///
/// # [`Elision`] 과 다른 필드인 이유
///
/// 둘은 다른 사건이다:
...
///
/// ```text
/// serialized_bytes  ← **잰 것.** 이 응답 묶음을 JSON 으로 쓴 바이트 수
/// bytes_per_token   ← **가정한 것.** 토크나이저가 아니다
/// approx_tokens     ← 위 둘에서 나온 것
/// ```
///
/// **`serialized_bytes` 는 이 필드가 채워지기 전의 응답 묶음을 잰 값이다.** 자기 자신을
/// 세는 것은 불가능하고, 그 차이는 소비자가 밖에서 잴 수 있다(`[f06.2.pass]` ③).
///
/// # **하한이다** — 그리고 그 사실이 표시되어야 한다
///
/// 재는 것은 **빈틈 없는 JSON** 이다. 표면이 사람을 위해 들여쓰면 실제로 나가는 텍스트는
/// 이보다 크다. 들여쓰기는 응답 묶음의 내용이 아니라 **표면의 선택**이라 여기서 못 안다.
/// 옛 백서 §6.3 이 요구하는 것이 정확히 이 형태다 — *"하한임이 표시되어야 한다."*
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
/// `&'static str` — **이 빌드에 박힌 상수**이기 때문이다. 밖에서 온 응답 묶음의 능력 목록을
