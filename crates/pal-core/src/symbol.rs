...
pub struct Symbol {
    ...
    /// **선언 밖 장식**이 변했는가에 답하는 값 — [`body`] 가 못 보는 축 (#77).
    ///
    /// Rust 의 속성(`#[derive]` · `#[cfg]` · `#[serde(…)]` · `#[must_use]`)은 선언 마디의
    /// **앞 형제**라 span 에 안 들고(span 을 넓히면 서술물 자리 맵이 갈린다 · #66), 메서드
    /// 수신자의 `&` 는 정규화가 선행 구분자로 벗긴다. 그래서 둘 다 [`body`] 에서 안 보였고,
    /// 그 변경에 결박이 `fresh` 로 남았다.
    ///
    /// **[`body`] 에 섞지 않는다** — 섞으면 이미 선 결박의 기준값이 옛 규칙 값이라 코드가
    /// 안 바뀌었는데 `stale` 로 뒤집힌다([`crate::DecorBaseline`] 머리).
    ///
    /// 장식이 선언 마디 **안**에 있는 언어(TypeScript 데코레이터 · Kotlin 애너테이션)는
    /// 이미 [`body`] 가 보므로 **빈 장식의 요약**이다. 없는 것이 정확한 값이다.
    ///
    /// [`body`]: Symbol::body
    pub decor: crate::coord::BodyDigest,
