...
pub enum ExtractGrade {
    ...
    /// 스코프 해소된 참조.
    ///
    /// ⚠ **이 등급이 스코프 체인의 유무를 지지 않는다** (2026-09-08 · #130).
    /// Rust 는 `L1` 인 채로 스코프 체인을 세워 파일 안 참조를 해소한다. 이 등급이
    /// 실제로 지는 것은 **`body_digest` 가 지역 이름을 지우는가**(`IdentityGrade`)이고,
    /// 그것은 리네임에 흔들리지 않는 요약을 만드는 대신 **결박을 이동시키는** 대가를
    /// 치른다. 두 축이 갈린 사연은 `pal_extract::grade_of` 옆에 있다.
    L2,
    /// 정의-사용(읽기/쓰기 집합).
    L3,
    /// 제어흐름(경로·지배관계).
