//! 결박 반경 — **닫히지 않는 문제를 선언으로 다룬다** (옛 F09 §3).
//!
...
//!
//! **반경이 판정 결과에 함께 출력된다.** *"이 결정은 `symbol` 반경에서 fresh"* 는
...
/// 감시 집합을 산출한다. **대상은 언제나 들어 있다.**
///
/// # 정렬한다
///
/// [`BTreeSet`] 을 쓰므로 회차마다 같은 순서가 나온다. 순서가 흔들리면
/// `watch_snapshot` 의 바이트가 흔들리고, 그러면 **왕복 항등이 성립하지 못한다**
/// (`[f09.3.pass]`).
///
/// # 빈 감시 집합은 만들 수 없다
///
...
/// 저장 시점의 예산 판정 — **런타임에 조용히 느려지는 대신 여기서 실패한다** (옛 F09 §3).
///
...
impl std::fmt::Display for BudgetRefusal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
             ...
             **저장 시점에 거부한다** — 런타임에 조용히 느려지는 것보다 낫다(옛 F09 §3). \
