//! 재결박 **제안 신호** — 옛 F03 §4.2 · §7.
//!
//! # 이 파일에 적용하는 함수가 없다. 그것이 대응이다
//!
//! 옛 F03 §5 는 *"파일 이동 시 자동 재결박"* 을 기각했다 — **의미가 다른 동일 코드에
...
/// 관측 가능한 사건이 되고, 옛 F03 §4.2 가 그것을 *"조용한 정체성 유실보다 낫다"* 로
/// 판단했다.
///
...
///
/// 결박이 좌표만 들고 있으면 [`propose`] 가 산출할 수 있는 것은 요약뿐이다. 이름과
...
/// # 왜 접두어인가
///
/// 옛 F09 §5 가 [R-08](../../../docs/plan/00-risks.md#r-08)(`Orphaned` 폭발)의 원인을
...
pub enum BatchRefusal {
    ...
    /// (이 파일 머리 · 옛 F03 §4.2). **일괄에서는 그 위험이 N 배가 된다.**
...
///
/// 옛 F03 §5 가 기각한 것은 *"의미가 다른 동일 코드에 **잘못 붙는** 것"* 이고,
...
mod batch_tests {
    ...
    fn 신호가_약하면_묶음_전체를_거부한다() {
        ...
        // 흔하고, 일괄에서는 그 위험이 묶음 크기만큼 곱해진다(옛 F03 §5).
        let b = 묶음(vec![
