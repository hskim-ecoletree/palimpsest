...
/// 상태다(아직 아무도 안 걸었다). 깨졌으면 **오류다** — 옛 F05 §6 이 못 박았다:
...
impl IntentStore {
    ...
    /// *"결박 0 건"* 이 나오고 그것이 옛 F05 §6 이 금한 형태다.
    ///
    /// # Errors
    ...
    /// **이 심볼들을 지켜보는** 결박 전부 — 증분 상태 갱신의 입구 (옛 F09 §4.1).
    ///
...
// ─────────────────────────────────────────────────────────────────────────────
// JSONL 내보내기/읽기 — **이 저장소만 지는 부담이다** (옛 F05 §2 의 넷째 · §6 의 표)
//
