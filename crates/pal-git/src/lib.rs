...
impl WorktreeState {
    ...
    /// 있으면 *"내가 잰 그 워킹트리인가"* 다.
    ///
    /// ⚠ **뒤의 것은 `--at` 없는 경로에서 원리상 발화하지 못한다.** ⟨`MS-10` · 2026-09-12⟩
    /// 앞 판은 여기에 *"대장을 계산하는 동안 사용자가 파일을 고칠 수 있고, 그러면 요약이
    /// 달라진다"* 를 이 함수가 잡는 위험으로 적었다. **그 보증은 없다** — `--at` 이 없으면
    /// `pal-cli` 의 `ledger.rs` 가 `None => worktree.tree_ref()` 로 **같은 `WorktreeState`
    /// 에서** `TreeRef` 를 짓고, 아래 `Worktree` 가지가 그 `tree_digest` 를 `self.tree_digest`
    /// 와 견준다. **두 변이 같은 필드라 언제나 참이다.** 이 가지가 무언가를 재려면 `TreeRef`
    /// 가 **다른 시점**에 떠 있어야 하고, 그 자리는 `--at` 을 준 경로뿐이다.
    ///
    /// ★ `PM2-04`(더러운 워킹트리에 「일치」를 찍는다)와 **다른 관측**이다 — 그쪽은 화면
    /// 문자열이고 이쪽은 **주석이 문서화한 보증의 부재**다.
    #[must_use]
