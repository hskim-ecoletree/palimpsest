...
//! 세우면 재구축이 그것을 지우고 `touch` 가 조용히 빈 결박을 산출한다 —
...
//! 응답 묶음의 `projection.rebuild` 가 `NotBuilt{F05}` 였던 자리가 값이 된다
//! (옛 DESIGN §12.7 격리 3번).
...
pub struct StitchReport {
    ...
    /// 파일 **경계를 넘은** 엣지. [`Self::edges`] 와 갈라 헤아린다.
    ///
...
impl Projection {
    ...
    /// 2층에 들어 있는 심볼 수. 응답 묶음의 `projection.symbols_indexed` 가 이것이다.
    ///
    /// # Errors
    /// 읽기가 실패하면.
    ...
    /// 뜻했습니까"* 뿐이고, 후보의 좌표는 사용자가 하나를 고른 **다음** 질의가 산출한다.
    ...
    /// 빈 답 규율이다. **자르는 것은 후보를 고른 뒤이고 그 생략은 `elision` 에 실린다.**
    ///
    /// # Errors
    /// 읽기가 실패하면.
    ...
    /// 우리 답과 댄다. **이 기능에서 유일하게 바깥에 있는 대조 도구다**(R-18).
    ///
    /// # Errors
    /// 읽기가 실패하거나 값을 풀지 못하면.
    ...
    /// 엣지 수 — **한 방향만 잰다.** 두 방향의 수가 같아야 하고 그것이 `[f05.2.pass]` ① 이다.
    ///
    /// # Errors
    /// 읽기가 실패하면.
    ...
    ///
    /// **[`None`] 은 *"안 세워졌다"* 이지 *"아무 스냅샷"* 이 아니다.** 응답 묶음의
...
//
// # 오류를 빈 목록으로 뭉갠다 — 그리고 그것이 안전한 방향인 이유
//
...
// 그것을 산출한다. **`callers` 반경인데 크기가 1 이면 산출에서 보인다.**
...
// 그 축소는 조용하지 않다: 미결박은 `narrative.unbound` 가 **작업 목록으로** 제출한다.
//
// ⚠ **후보 수를 자르지 않는다.** 자르면 *"여럿이라 못 좁혔다"* 의 규모가 사라지고,
// 사람은 **50 건짜리 제안과 2 건짜리 제안을 같은 화면으로 본다.** 후보의 수 자체가
// 산출이다(`TouchAnswer::Ambiguous` 와 같은 판단) — 조용한 생략 금지.
// ─────────────────────────────────────────────────────────────────────────────
