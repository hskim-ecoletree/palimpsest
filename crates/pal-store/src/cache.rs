...
/// 1층 캐시의 **포트** — stack §5.2 · 옛 F04 §2.
///
/// # 왜 구체 타입 하나로 두지 않는가
///
/// 옛 F04 §5 가 `rkyv` 를 기각하면서 *"postcard 로 시작하고 병목이 확인되면 교체 —
    ...
    fn entries(&self) -> Result<(Vec<Scanned>, u64), CacheError> {
        ...
        for shard in shards {
                ...
                .map_err(|e| CacheError::Read(format!("{}: {e}", shard.display())))?
            {
                ...
                // **접근 시각을 mtime 으로 근사한다**(옛 F04 §3.4) — `noatime` 마운트에서도
...
impl ExtractCache for BlobCache {
    ...
    /// # 왜 정책이 이렇게 단순해도 되는가
    ///
    /// 옛 F04 §3.4: *"축출은 정확성에 영향이 없다 — 재파싱될 뿐이다."* 틀린 것을 지울
