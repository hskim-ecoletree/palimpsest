...
//!   ([`crate::install::hooks`]). **stdin 바이트는 shell form 과 SHA-256 까지
//!   동일하다** — 그래서 이 파일의 규약은 형태가 바뀌어도 안 움직인다.
...
//!
//! # ★ transport 실패와 활성 정책의 상태 실패는 다르다
//!
//! 이 파일의 transport 실패는 **조용한 통과**다 — JSON 이 아닌 입력 · 모르는 사건 ·
//! 못 읽은 표준입력은 차단하지 않고 까닭만 표준오류에 남긴다. 반대로 명시적으로 활성화된
//! `Stop` 의 payload·회차 상태가 손상됐다면 그것은 이미 아는 사건의 안전 판정이며 정책이
//! 차단한다. 등록만 된 비활성 `Stop` 은 여전히 통과한다.
//!
//! # ★ 그리고 훅 실행 실패는 침묵한다
//!
//! 실행 파일이 없거나 실행 권한이 없으면 세션은 계속되고 `claude` 의 종료 코드는
//! **0** 이다. 트랜스크립트에도 대화형 화면에도 흔적이 없다. ⚠ **exec form 에서는 한
//! 겹 더 어둡다** — shell form 이 내던 126/127 조차 안 나오고 종료 코드가 **항상 1**
//! 이다. 그래서 훅은 **부르면 언제나 표준오류에 한 줄**을 남기고, `pal doctor` 의
//! 검사가 그 줄을 찾아 「등록된 명령이 실제로 도는가」를 가른다.
