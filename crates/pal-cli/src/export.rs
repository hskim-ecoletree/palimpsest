//! `pal export` — **자유 탐색을 여는 자리** (F06 §4.1 · `[f06.3.pass]` ④).
//!
//! # 이 명령이 검사하는 것은 F05 의 근거 하나다
//!
//! F05 §2 가 그래프 DB 를 안 쓰기로 한 근거 다섯 중 마지막이
//! *"그래프 DB 방향은 `pal export`(Cypher/GraphML/Parquet)로 만족시킨다"* 였다.
//! **그 만족이 없는 동안 그 근거는 검사되지 않는 주장이었다.** 형식 하나만 서면
//! 주장이 관측이 된다.
//!
//! 그리고 F06 §6 이 자유 질의 언어 노출(Cypher 패스스루)을 기각하면서 적었다 —
...
/// F06 §8 이 *"형식 하나만이라도"* 라고 적었고, 하나가 서면 F05 의 근거가 검사된다.
///
