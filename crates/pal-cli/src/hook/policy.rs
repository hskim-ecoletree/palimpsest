...
//! # 사건 등록과 정책 활성화는 다르다
//!
//! [`super::catalog`] 는 하네스가 받을 수 있는 사건과 dispatch 만 안다. `Stop` 이 거기에
//! 등록돼 있다는 사실은 차단 승인이 아니다. 프로젝트별 명시적 activation 은
//! [`crate::round::stop`] 이 읽는 private store 에 따로 있고, 없으면 기존 동작 그대로
//! 통과한다. 이 파일은 두 정책으로 보내는 한 자리이며 규약·설치·진단은 밖에 있다.
//!
//! # SubagentStop — **돌려준 말이 비어 있는 서브에이전트**
//!
...
pub fn decide(event: &str, payload: &Value) -> Decision {
    ...
    match spec.policy {
        Policy::SubagentStop => decide_subagent(payload),
        Policy::RoundStop => match crate::round::stop::decide(payload) {
            crate::round::stop::PolicyDecision::Pass(why) => Decision::Pass(why),
            crate::round::stop::PolicyDecision::Block(reason) => Decision::Block(reason),
        },
    }
}

fn decide_subagent(payload: &Value) -> Decision {
    match payload.get("last_assistant_message") {
        None => Decision::Pass(
            "돌려준 말이 페이로드에 없다 — 모르는 형태에서 차단하지 않는다".to_owned(),
        ),
        Some(Value::String(s)) if s.trim().is_empty() => Decision::Block(빈_말.to_owned()),
        Some(Value::String(_)) => Decision::Pass("돌려준 말이 있다".to_owned()),
        Some(_) => Decision::Pass(
            "돌려준 말이 문자열이 아니다 — 모르는 형태에서 차단하지 않는다".to_owned(),
        ),
    }
}
