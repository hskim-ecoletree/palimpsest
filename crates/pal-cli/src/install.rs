...
//! `cargo xtask check` 의 열여섯째 검사가 잰다. ⚠ 그러나 **문자열 스캔만으로는
...
impl Lock {
    ...
    /// 언제나 1개.** 배타가 이 플랫폼에서 실제로 성립한다 — 앞 회차가 *"거의 안 난다"* 로
    /// 적어 둔 것이 이제 **잰 것**이다.
    ///
    /// 그러니 여기서 `true` 를 산출하는 것은 「못 재서 통과」가 아니라 **「그 경합이 없다」**
...
pub fn install(target: &Path) -> Result<()> {
    ...
    // ★ **되돌릴 수 없는 블록 위에 설치하지 않는다 — 그리고 그 판정이 1단계에 놓인다.**
...
fn 쓸_수_있나(root: &Root) -> Result<Vec<String>> {
    ...
    let mut 알림 = Vec::new();
    // ★ **못 세는 플랫폼은 「늘 끊는다」를 한 줄로 출력한다.** 자리마다 내면 평범한 설치가
        ...
        .chain(DIRS.iter().copied())
    {
        ...
        // ★ **파일시스템이 답을 다르게 산출하는 자리는 1단계에서 끊는다.** 대소문자만 다른
        // 이름이 이미 있으면 우리 블록이 어디로 들어가는지가 **플랫폼마다 다르고**,
        // 그러면 공유되는 저장소가 clone 한 곳에 따라 다르게 잡힌다([`casing`]).
...
fn 쓸_수_있는가(path: &Path) -> Result<()> {
    ...
    // [`하드링크_알림`] 을 [`쓸_수_있나`] 가 모아서 화면에 출력한다.
    //
...
/// 이전 매니페스트를 읽되 **경계 검사를 지난 것만 산출한다.**
///
...
/// **되돌릴 수 없는 블록이 이미 있는가** — ★ **1단계에서 본다.**
///
/// # 같은 판정이 이미 있었다. 그런데 너무 늦게 서 있었다
///
/// [`블록_하나`] 가 *"우리 마커가 있는데 매니페스트에 그 기록이 없다"* 와 *"기록은
/// 있는데 실물이 다르다"* 둘을 이미 거부한다. 그러나 거기까지 오면 **디렉터리와 파일
/// 다섯과 설정이 이미 쓰인 뒤**다 — 그 `bail!` 이 rc=1 을 내면서 **새 부분 설치를
/// 남겼다**(실측). 게이트 ② 가 재는 것이 정확히 그 형태다: *"실패한 설치가 사용자
/// 프로젝트를 반쯤 오염시키면 `doctor` 도 `uninstall` 도 그 상태를 모른다."*
///
/// 판정을 여기로 앞당긴다. **여기서 멈추면 한 바이트도 안 썼다.** [`블록_하나`] 의
/// 문은 그대로 둔다 — 1단계와 적용 사이에 남이 파일을 바꿀 수 있고, 그 자리를 지는
/// 것은 여전히 그 문이다.
///
/// # Errors
/// 마커가 있는데 기록이 없거나, 기록이 있는데 실물이 우리 것과 다르면.
fn 되돌릴_수_있나(root: &Root, 이전: Option<&Manifest>) -> Result<()> {
    for name in [ROOT_INSTRUCTION_FILE, IGNORE_FILE] {
        let rel = Rel::new(name);
        let path = root.join(&rel)?;
        // ⚠ **일반 파일이 아닌 자리는 여기서 판정하지 않는다.** 거기엔 블록이 없고,
        // 그 자리가 왜 안 되는지는 **읽고 쓰는 문**([`guard`])이 뒤에서 말한다. 여기서
        // 대신 끊으면 *"미리 못 보는 실패도 기록을 남긴다"*(기록 (b))가 재는 자리가
        // 사라진다 — 이 함수가 지는 질문은 **되돌릴 수 없는 블록이 있는가** 하나다.
        if !path.is_file() {
            continue;
        }
        let markers = 마커(&rel);
        match 이전.and_then(|m| m.blocks.iter().find(|b| b.path == rel)) {
            // 기록이 없으면 물을 수 있는 것은 **마커가 있는가** 하나다.
            None if blocks::마커가_남았나(&path, markers)? => {
                bail!("{}", 기록_없는_블록_문구(&rel, markers));
            }
            Some(old) if blocks::상태(&path, markers, &old.inserted)? == blocks::상태::훼손 => {
                bail!("{}", blocks::훼손_문구(&path, markers));
            }
            _ => {}
        }
    }
...
/// **되돌리기 기록을 잃은 블록에서 빠져나오는 길.**
///
/// 우리 마커가 파일에 있는데 매니페스트에 그 기록이 없으면 제거가 그것을 못 걷어내고,
/// 그때 `uninstall` 은 **rc=0 으로 「제거」 화면을 내면서 블록을 남긴다.**
///
/// ★ **무엇을 지워야 하는지 이름으로 말한다.** 옛 문구는 *"그 블록을 손으로 지운 뒤"*
/// 였는데 **무엇이 「그 블록」인지 안 알려 줬다** — [`blocks::빠져나오는_길`] 이 그
/// 두 줄을 준다.
fn 기록_없는_블록_문구(rel: &Rel, markers: &layout::Markers) -> String {
    format!(
        "{rel} 에 우리 마커가 있는데 매니페스트에 그 기록이 없다 — **제거가 이 블록을 \
         못 되돌린다.**\n    \
         성공이라고 적지 않는다. 기록이 살아 있는 매니페스트를 되살리거나, {}",
        blocks::빠져나오는_길(markers, "pal install")
    )
}
...
fn 파일_놓기(root: &Root, 기록: &mut Journal, report: &mut Report) -> Result<()> {
    for res in PAYLOAD {
        ...
        // **실물에서 뜬다** — 매니페스트가 적는 값이 곧 디스크의 값이어야 ③ 이 성립한다.
...
fn 설정_병합(
    ...
    // **적어 둔 것과 바라는 것을 대서 계획을 산출한다.** 실행 파일이 옮겨 갔으면 옛 등록이
...
fn 블록_넣기(
    ...
    if let Some(old) = 옛_등재 {
        ...
        match blocks::상태(&path, 마커(&ignore_rel), &old.inserted)? {
            ...
            // **거짓 성공을 보고하지 않는다.** 마커가 남았는데 우리 바이트가 아니면 제거가
    ...
    match blocks::add(&path, 마커(&rel), block)? {
        ...
        blocks::Added::AlreadyThere => {
            ...
            let Some(old) = 옛것 else {
                // ★ **되돌리기 기록을 잃은 채 성공을 보고하지 않는다.**
                // 같은 판정이 [`되돌릴_수_있나`] 에서 1단계에 이미 섰다 — 여기는 그
                // 사이에 남이 파일을 바꾼 자리를 받는다.
                bail!("{}", 기록_없는_블록_문구(&rel, 마커(&rel)));
            };
...
pub fn uninstall(target: &Path, 선택: 제거) -> Result<()> {
    ...
    // 자리는 통과로도 반증으로도 안 잰다」). 그때 매니페스트는 *"놓았다"* 고 주장한 적이
...
fn 디렉터리_걷기(m: &Manifest, 자리: &manifest::Places, report: &mut Report) -> Result<()> {
    ...
    // `manifest::walk` 가 그 규칙 하나 위에 성립한다). 깊이는 그 개수다.
...
fn 안쪽을_걷는다(안: &안쪽, report: &mut Report) -> Result<()> {
    ...
    for 자리 in &안.정본 {
        if !안.purge {
            ...
            // ★ **파일마다 한 줄씩 더 말한다.** 자리 한 줄로만 말하면 무엇이 남았는지 경로로 알 수 없다 —
            //   `--purge` 는 지운 정본을 파일마다 말하므로 남길 때도 같은 규율이다(F1 ②).
            for f in &자리.파일 {
                report.say("남겼다(정본)", &f.rel);
            }
            continue;
        }
        if 안.못_물음 {
