...
//!
//! # 왜 `pal-core` 가 아니라 여기인가 — 두 사고를 갈라 막는다
//!
...
//!
//! **①은 구조가 안 막는다.** `crate::export` 는 이 모듈과 **같은 크레이트의 형제**라
//! `crate::label::정체성_등급(…).병기()` 를 넣으면 컴파일된다 — 독립 리뷰 R1(발견 6)이
//! 격리 사본에서 그것을 실측했고, 넣은 채로 `cargo xtask check` 가 초록이었다.
//! **그래서 그 자리는 시험이 막는다** — `crates/pal-cli/tests/export_ascii.rs`.
//! 못 막는 것을 「막힌다」고 적으면 그것이 곧 거짓 신호다.
//!
//! # 두 조각으로 돌려주는 까닭
//!
