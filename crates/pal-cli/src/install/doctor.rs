...
/// 3. 그 자리가 **우리가 등록한 그 프로그램인가**([`hooks::우리가_등록한_것인가`] —
///    바이트 대조). ★ **이 겹이 없으면 앞의 셋이 전부 통과하면서 남의 실행 파일이
///    초록을 받는다**(실측): 훅의 `command` 만 `Rel` 이 아니라 경계 검사의 모집단
///    밖이다
/// 4. **훅 규약이 실제로 서는가** — [`hooks::probe`] 가 **지금 도는 이 실행 파일**을
///    셸 없이 띄운다
///
/// ⚠ **2·3 에서 읽은 것을 4 가 안 쓴다.** 그 경로는 남이 커밋해 보낸 파일에서 왔고,
/// 옛 코드는 그것을 `/bin/sh -c` 로 돌려서 `pal doctor` 한 번이 임의 코드 실행이었다.
/// 3 이 하는 것은 **읽어서 대는 것**뿐이다 — 그리고 3 이 서야 **4 의 증거가 등록된
/// 것에 대해 말한다.**
fn 훅(root: Option<&Path>) -> Outcome {
    ...
    for h in 적힌 {
        ...
        // ★ **하네스가 실제로 띄울 것이 우리가 등록한 그것인가.**
        //
        // 훅의 `command` 만 `Rel` 이 아니라 경계 검사의 **모집단 밖**이다. 그 한 칸이
        // 비어서, 매니페스트와 설정의 `command` 를 **대상 밖 실행 파일**로 맞춰 두면
        // 이 검사가 초록이었다(실측) — 아래 셋이 전부 통과하기 때문이다: 등록이 설정과
        // 맞고(문자열 대조) · 우리 형태이고(`args`) · 그 자리가 실행될 수 있다(`stat`).
        // **`command` 그 자체를 아무도 안 봤다.**
        if let Err(e) = hooks::우리가_등록한_것인가(&등록된_자리) {
