//! **`\\?\` 를 사람 앞에 안 보인다** — 그러나 그것이 필요한 자리에서는 안 벗긴다.
//!
...
//! ```
//!
//! [`std::fs::canonicalize`] 는 Windows 에서 **verbatim 형태**(`\\?\`)를 산출한다. 우리는
...
//! - 화면의 한 줄이 유닉스와 **다르게 생긴다.** 이 회차의 문장은 *"같은 결과를 산출한다"* 다.
//!
...
/// 사람에게 산출할 경로 문자열 — **`\\?\` 를 뗀다(뗄 수 있을 때만).**
///
