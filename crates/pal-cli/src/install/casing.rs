//! **이름이 대소문자만 다른 것과 부딪히는가** — 파일시스템이 답을 다르게 출력한다.
//!
//! # 왜 이 문이 필요한가
//!
...
//! | 양쪽 다 **있는 이름을 따른다** | 리눅스에서는 하네스가 안 읽는 파일에 쓰게 된다 — 설치가 **아무 효과가 없는 상태**를 rc=0 으로 산출한다 |
//! | 양쪽 다 **멈추고 말한다** ← 고른 것 | 가능하다. 탐지가 [`std::fs::read_dir`] 위에 서고 그것은 어디서나 같은 답을 돌려준다 |
//!
//! ⚠ **잃는 것을 적어 둔다.** 리눅스에서 `Claude.md` 라는 **무관한 파일**을 가진
//! 사용자가 설치를 못 한다. 그 사람이 할 일은 `git mv` 한 번이고, 그 대신 얻는 것은
//! *"이 저장소를 세 플랫폼 중 어디서 clone 해도 같은 것이 나온다"* 이다. 그리고 그
//! 사람의 저장소는 **우리와 무관하게 이미** Windows 동료에게서 깨져 있었다 — 이
//! 문은 그 사실을 설치 시점에 보이게 만들 뿐이다.
//!
//! # ⚠ ASCII 범위에서만 대소문자를 맞춘다
//!
//! [`str::eq_ignore_ascii_case`] 를 쓴다. 우리가 놓는 이름은 전부 ASCII 이고
//! (`CLAUDE.md`·`.gitignore`·`.claude`…), 유니코드 대소문자 맞춤은 **파일시스템마다
//! 규칙이 다르다**(APFS 는 NFD 정규화까지 한다). 우리 이름 밖의 맞춤 규칙을 흉내 내면
...
/// `read_dir` 은 **디스크에 적힌 그대로**를 산출한다(NTFS·APFS 는 대소문자를 보존한다).
...
mod tests {
    ...
    /// ★ **어느 플랫폼에서나 같은 답을 돌려준다** — 그것이 이 모듈의 존재 이유다.
    ///
    ...
    /// `.gitignore.bak` 에 걸린다 — 그러면 이 문이 거짓 거부를 산출한다.
    #[test]
    fn 다른_이름은_안_걸린다() {
        let d = 방("무관");
