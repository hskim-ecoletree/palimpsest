...
/// **파일 내용의** sha256 — 매니페스트가 적고 대조가 대는 값은 전부 이것이다.
///
/// # 왜 [`hex`] 와 이름을 갈랐는가
///
/// `hex` 는 *"이 바이트열의 sha256"* 이고, 이것은 *"이 파일의 **내용**의 sha256"* 이다.
/// 둘은 같은 함수일 수도 있고 아닐 수도 있다 — **그 결정이 한 자리에 있어야** 나중에
/// 「무엇을 같다고 볼 것인가」가 흩어지지 않는다. 지금 이 자리를 지나는 것은 넷이다:
/// [`super::install`] 의 놓기 · `update` 의 3분기 · `uninstall` 의 「사용자 수정」 판정 ·
/// [`super::manifest::walk`] 의 실물 훑기.
#[must_use]
pub fn 내용(bytes: &[u8]) -> String {
    hex(bytes)
}
