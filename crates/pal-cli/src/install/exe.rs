//! **「이 자리가 실행될 수 있는가」** — 플랫폼이 답을 다르게 출력한다.
//!
//! # 왜 한 자리에 모으는가
//!
...
//! | ACL 로 실행 권한을 본다 | std 에 문이 없고, `unsafe 금지` 게이트가 raw FFI 를 막는다. **없는 것을 근거로 삼지 않는다** — 대신 [`권한_겹`] 이 그 부재를 화면에 출력한다 |
...
// **`Option` 은 플랫폼마다 한쪽으로 뭉개진다.** 한 플랫폼만 보면 언제나 `Some` 이거나
