...
impl Root {
    ...
    /// 대상 안의 자리 하나.
    ///
    /// **밖으로 나가면 실패한다.** 나가는 항목은 **건드리지 않고** 그 사실을 산출한다 —
    ...
    pub fn join(&self, rel: &Rel) -> Result<PathBuf> {
        if let Some(까닭) = 글자로_벗어나나(&rel.0) {
            bail!(
                "`{rel}` 은 **대상 밖**이다 ({까닭}) — 대상은 {self} 다.\n    \
                 매니페스트는 대상 프로젝트 안에 있는 파일이고, 그것이 적은 경로가 밖을 \
        ...
        // ★ **확정한 것이 아니라 원래 경로를 산출한다.** 확정한 것을 쓰면 대상 **안**을
...
/// ★ **화면에는 `\\?\` 를 내놓지 않는다.** [`Root::세운다`] 가 `canonicalize` 를 지나므로
