...
//! | 후보 | 왜 안 골랐나 |
//! |---|---|
//! | 등록을 `.claude/settings.local.json` 으로 옮긴다 | **구조로는 이쪽이 더 맞다** — 기계 고유의 사실은 기계 고유의 파일에 살아야 하고 하네스가 그 층을 이미 갖고 있다. 그런데 **그 층의 훅이 실제로 발화하는지를 이 저장소가 안 쟀다.** 재려면 실제 `claude` 세션을 몰아야 하고 이 회차는 그것을 못 한다. **안 잰 것 위에 경계를 세우지 않는다** |
