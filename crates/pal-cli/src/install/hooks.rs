...
//! 경로로 등록한다."*
//!
//! **그 근거는 한 기계만 볼 때만 성립한다.** `settings.json` 과 매니페스트는 **커밋되고
...
/// *"자리들에 넣자"* 가 첫 생각이 되는데 **그 문은 여기 원리상 성립하지 않는다** — 우리가
...
///
/// ★ **[`probe`] 는 [`std::env::current_exe`] 를 돌려서 「훅 규약이 성립한다」고 말한다.
...
/// 지금 설정 · 우리가 적어 둔 것 · 바라는 것 셋을 대서 계획을 산출한다.
#[must_use]
pub fn plan(
