...
//! `.claude/agents/` 는 **남의 에이전트가 함께 쓰는 자리**라 훑으면 남의 것을 우리 것으로
//! 잰다 — 그래서 그쪽만 **파일 하나짜리 뿌리**다.
...
pub struct Manifest {
    ...
    /// 못 찾았다"* 를 산출한다 — 자기가 지운 자리를 보고 산출하는 거짓 경보이고, 그것이
    ...
    if layout::절대_안_건드리나(s) {
        bail!(
            "`{rel}` 은 **어떤 경우에도 안 건드린다** — 매니페스트가 그것을 적었다는 \
             ...
             매니페스트는 대상 프로젝트 안에 있는 파일이고 커밋과 함께 이동한다"
        );
    }
    let 된다 = match 종류 {
    ...
    if !된다 {
        bail!(
            "`{rel}` 은 **우리가 놓을 수 있는 자리가 아니다**({종류:?} 로 적혔다) — \
             건드리지 않는다.\n    \
             매니페스트는 대상 프로젝트 안에 있는 평범한 파일이라 남이 커밋해 보낼 수 \
...
/// 매니페스트가 지는 **경로 항목 수의 상한.**
///
/// # 왜 상한이 필요한가 — 경계 검사가 못 막는 자리다
///
/// 매니페스트는 대상 안의 **커밋되는 평범한 파일**이라 **크기가 남의 손에 있다.**
/// 그리고 크기로 거는 공격은 항목 하나하나가 **전부 정당한 경로**라 [`우리_자리인가`]
/// 가 원리상 못 막는다. 실측(고치기 전): 3,200개에서 `uninstall` 이 **18.1초**,
/// 20,000개는 60초에 미완료였다.
///
/// # 왜 256 인가
///
/// **우리가 놓는 것은 실측 20 이다**(2026-08-19). 상한은 그것의 **약 13배**다.
/// 리소스가 지금의 열 배로 늘어도 안 걸리고, 그 위는 우리가 놓은 것이 아니다.
///
/// ⚠ **이 수를 어디에도 베끼지 마라.** 앞 판은 「17개」였고 `PAYLOAD` 가 늘자 갈렸다 —
/// 그런데 `tests/install_hostile.rs` 가 그 17 을 **베껴서 230 을 채웠고**, 실제가 20 이
/// 되자 `230 + 20 + …` 이 상한을 넘어 **CI 세 OS 가 전부 빨개졌다**(실측 259).
/// **세는 자리는 [`Manifest::경로들`] 하나다** — 시험도 그것을 세야 한다.
///
/// ⚠ **상한만으로는 부족하다.** 상한 바로 아래에서 O(n²) 가 남아 있으면 같은 형태가
/// 그대로 산다 — 그래서 [`super::uninstall`] 의 걷기도 **항목 수에 선형**으로 고쳤다.
/// **둘 다** 있어야 이 자리가 닫힌다.
pub const 항목_상한: usize = 256;

/// 매니페스트를 읽는다.
///
/// # Errors
/// 없거나, 못 읽거나, JSON 이 아니거나, **항목 수가 [`항목_상한`] 을 넘으면.**
/// **손상된 매니페스트를 조용히 덮지 않는다** — 그것을 덮으면 *"무엇을 되돌려야
/// 하는가"* 의 기록이 사라진다.
pub fn read(path: &Path) -> Result<Manifest> {
    ...
    if 몇 > 항목_상한 {
        bail!(
            "{} 가 경로 {몇}개를 적었다 — **상한 {항목_상한}개를 넘는다.**\n    \
             우리가 놓는 것은 **17개**다. 이 파일은 대상 프로젝트 안에 있는 평범한 \
             파일이라 남이 커밋해 보낼 수 있고, **크기 자체가 무기가 된다** — 항목이 \
             전부 정당한 경로라 자리 검사로는 안 걸린다.\n    \
             건드리지 않는다. 사람이 봐야 한다",
            path.display()
        );
    }
...
mod tests {
    ...
    fn 경로를_하나도_안_빠뜨린다() {
        ...
        assert_eq!(실린, 적힌, "직렬화에는 있는데 `경로들` 이 안 산출하는 경로가 있다");
    }
    ...
    /// **적혔는데 없는 것**과 **생겼는데 안 적힌 것**을 갈라 잰다.
    #[test]
    fn 양방향을_갈라_잰다() {
        let recorded = 적힌(&[("x", "1")]);
        let actual = 실험용(&[("y", "2")]);
