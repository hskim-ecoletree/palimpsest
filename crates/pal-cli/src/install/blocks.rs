...
/// 지우면 남은 벌이 같은 판정을 또 산출한다 — 그러면 문구가 말한 길이 **실제로는 없는
...
///
/// `core.autocrlf` 가 걸린 클론에서는 우리가 넣은 블록이 CRLF 로 앉아 있고, 매니페스트
...
/// **있는 파일에 제자리로 쓴다** — 모드와 안쪽 심링크가 살아 있는 유일한 길.
///
/// ⚠ **하드링크가 걸린 자리에는 그냥 안 쓴다**([`super::guard::제자리를_준비한다`]).
/// 제자리 쓰기가 살리던 것 셋 중 **하드링크만은 밖으로 새는 통로**여서, 그 하나를
/// 포기하고 경계를 지킨다 — 링크 수를 셀 수 있는 플랫폼은 **거부**하고, 셀 수 없는
/// 플랫폼은 **끊고 쓴다.** 보증은 같고 상호작용만 다르다.
///
/// # Errors
/// 열지 못하거나, 쓰지 못하거나, **하드링크를 거부하거나 끊지 못하면.**
pub fn write_in_place(path: &Path, bytes: &[u8]) -> Result<()> {
    super::guard::일반_파일이거나_없나(path)?;
    super::guard::제자리를_준비한다(path)?;
    let mut file = OpenOptions::new()
        .write(true)
        .open(path)
...
mod tests {
    ...
    /// 산출했다. 그래서 `uninstall` 이 rc=0 으로 *"블록 뺌"* 을 찍으면서 **완전한 블록을
