...
///   `ordinal` 이다.
///
/// ⚠⚠ **둘째를 「순서 위험이 없다」로 읽으면 안 된다.** `ordinal` 은 0 부터이므로 **중복
/// 그룹의 첫 선언도 `ordinal == 0` 이고 둘째 버킷으로 간다.** 그런데 [`SymbolId::compute`]
/// 가 `discriminator.ordinal` 을 해시에 넣으므로(`crates/pal-core/src/coord.rs`), 같은
/// 그룹의 선언 **순서를 바꾸면 그 첫 선언의 정체성도 맞바뀐다** — 같은 파일의 시험
/// `체인이_없으면_순서가_정체성을_흔든다` 가 그것을 이미 고정하고 있다.
/// **그러므로 이 버킷은 「이름이 유일한 것」과 「그룹의 첫 선언」을 함께 담는다.**
///
/// ★ 그래도 이 셈법을 쓰는 까닭은 [#79] 본문이 *"순서에 취약한 464 건(`ordinal>0`)"* 로
/// **자기 수를 그 술어로 정의**했기 때문이다. 셈법을 그룹 단위로 바꾸는 것은 그 이슈의
/// 닫는 조건을 다시 정의하는 일이라 **이 자리에서 안 한다** — 정반합 판 1 이 그것을
/// 소유자 승격으로 올렸다.
///
...
pub struct IdentityTally {
    ...
    pub 순서에_취약: usize,
    /// `ordinal == 0` 인데 추출기 등급이 `ordinal`.
    /// ⚠ **「순서 위험이 없다」가 아니다** — 중복 그룹의 첫 선언이 여기 섞인다(위 ⚠⚠).
    pub 등급이_낮음: usize,
    /// `ordinal == 0` 이고 추출기 등급이 `exact`.
    /// ⚠ 여기도 「이름으로 유일」이 아니다 — 같은 까닭이다(위 ⚠⚠).
    pub 정확: usize,
...
fn print_identity(report: &LedgerReport) {
    ...
    println!("  {:<16}{:>6}  {}", "", "", "   그 셈법은 `#79` 본문의 `ordinal>0` 을 그대로 씁니다 — **그룹 단위로 세면 값이 커집니다**");
    println!("  {:<16}{:>6}  {}", "등급이 낮음", t.등급이_낮음,
             "판별자가 순서로 안 가렸고 추출기가 스코프를 못 풀었습니다");
    // ⚠⚠ **「순서 위험이 없다」로 안 적는다.** 중복 그룹의 **첫 선언**이 이 칸에 섞이고,
    //   그것도 순서가 바뀌면 정체성이 맞바뀐다. 앞 판은 여기에 *"순서 위험은 없고"* 를
    //   **사실로** 찍었다 — 정반합 판 1 의 반(反)이 잡았다.
    println!("  {:<16}{:>6}  {}", "", "",
             "↑ 여기에 **중복 그룹의 첫 선언**이 섞입니다 — 그것도 순서가 바뀌면 정체성이 맞바뀝니다");
    // ⚠ 같은 까닭으로 **「이름으로 유일하다」라고 안 적는다** — `ordinal == 0` 은 「유일」이
    //   아니라 **그룹의 첫째**다. L2 언어에서 중복 그룹이 서면 이 칸에도 섞인다
    //   (지금 이 저장소에는 그 실례가 없다). 정반합 판 1 의 반(反) R6 이 잡았다.
    println!("  {:<16}{:>6}  {}", "정확", t.정확, "판별자가 순서로 안 가렸고 참조가 해소됩니다");
...
mod tests {
    ...
    fn 순서로_가린_것과_등급이_낮은_것이_갈린다() {
        ...
        // ⚠ **이 단언이 재는 것은 셈법이지 「순서 위험이 없다」가 아니다.** 첫 `dup` 은
        //   `ordinal == 0` 이라 이 버킷으로 가지만 **순서가 바뀌면 그 정체성도 맞바뀐다** —
        //   `체인이_없으면_순서가_정체성을_흔든다` 가 같은 파일에서 그것을 고정한다.
        //   앞 판의 메시지(*"첫 `dup` 과 `solo` 는 순서 위험이 없다"*)는 **거짓을 불변식으로
        //   못 박고 있었다** — 정반합 판 1 의 반(反)이 잡았다.
        assert_eq!(tally.등급이_낮음, 2, "`ordinal == 0` 인 둘이 이 버킷으로 가야 한다");
