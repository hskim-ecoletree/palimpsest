...
fn print_identity(report: &LedgerReport) {
    ...
    println!("  {:<16}{:>6}  {}", "", "", "↑ **겹친 자리의 심볼 전원**입니다(첫 선언 포함) — 선언 순서가 0 이 아닌 것보다 큽니다");
...
pub fn print_table(report: &LedgerReport) {
    ...
    // 담고 있지 않다는 뜻이다.
    //
    // ⚠⚠ **「워킹트리」라는 낱말이 이 제품의 표면 셋에서 서로 다른 것을 뜻한다** ⟨`MS-11`
    // · 2026-09-12⟩. 어느 산출도 그것을 안 가르므로 여기 한 자리에 적는다:
    //
    //   ⑴ 바로 위 `Snapshot … (워킹트리)` — **어느 시점을 보고 있나**의 축이다.
    //      커밋이 아니라 워킹트리를 떴다는 뜻이고 더러움과 무관하다.
    //   ⑵ 아래 `워킹트리  <sha> 와 같음 / 와 다른 파일 N개` — **더러움**의 축이다.
    //      ⑴ 이 `(커밋)` 이어도 이 줄은 다를 수 있다.
    //   ⑶ `pal touch` 근거 상자의 `워킹트리  일치 / 다름` — 그 표면의 축이고
    //      **더러운 워킹트리에서도 「일치」를 찍는다** ⟨`PM2-04` · 회차가 관측만 하고
    //      안 고치기로 한 자리⟩. ⑵ 와 같은 낱말이지만 **같은 것을 안 잰다.**
    //
    // ★ 셋을 한 낱말로 부르는 것이 이 관측의 내용이고, 고치는 자리는 표면 어휘 쪽이라
    //   이 회차의 재는 자리 밖이다. **여기서는 가르는 문장을 남기는 것이 처분이다.**
    ...
    println!();
    println!("provider  (이 빌드에 provider 포트가 없습니다)");
    println!("조달      (이 빌드에 관측 수용이 없습니다)");
