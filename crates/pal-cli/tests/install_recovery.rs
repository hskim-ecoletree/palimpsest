...
//! 20초를 기다린 뒤 rc=1 을 산출했다. 걷어낼 수 있다던 것을 **잠금이 못 걷게 했다.**
...
///
/// 관측(고치기 전): `update` **만** rc=0 을 **0초**에 산출했다 — 반쯤 설치되고 잠긴
...
/// *"하나도 못 찾았다"* 가 **자기가 지운 자리를 보고** 거짓 경보를 산출했다.
#[test]
fn 중간에서_실패한_제거가_이어서_끝난다() {
    ...
    assert!(!out.status.success(), "걸림돌을 보고도 성공을 돌려줬다");
...
fn 블록을_고쳐도_문구대로_하면_빠져나온다() {
    ...
    let out = 돌린다(&root, &["uninstall"]);
    assert!(!out.status.success(), "훼손된 블록을 보고도 성공을 돌려줬다");
    ...
    // ② `install` 도 **거짓 성공을 보고하지 않는다.**
    let out = 돌린다(&root, &["install"]);
    assert!(
        !out.status.success(),
        "고쳐진 블록을 보고도 rc=0 「설치」 화면을 산출했다\nstdout: {}",
        String::from_utf8_lossy(&out.stdout)
    );
...
/// 것은 **성공을 보고하지 않는다**뿐이다.
#[test]
fn 걷어내도_마커가_남으면_제거가_성공을_안_산출한다() {
    let root = 프로젝트("남은마커");
    let 원본 = "# 내 지시\n";
    std::fs::write(root.join("CLAUDE.md"), 원본).expect("CLAUDE.md");
    let s0 = 스냅샷(&root);
