//! **이관(fold)은 생략이 아니다** — `[f06.2.pass]` ①②③④ 와 `[f06].the_whole_surface_carries_an_envelope`.
//!
//! # 성분 아홉을 **코드에 상수로 박는다**
//!
...
/// 응답 묶음의 성분. **하나라도 빠지면 실패다.** F05 의 여섯 + F06 의 셋.
const 응답묶음의_아홉: [&str; 9] = [
    "snapshot",
    "projection",
    "coverage",
    "capabilities",
...
/// 대장이 안 뭉개지는 유일한 질의. **하나와 나머지가 다른 것이 이 시험의 요점이다.**
...
fn 큰_저장소(tag: &str) -> PathBuf {
    ...
    // 그러면 생략을 일으킬 수 없어 ④가 아무것도 못 잰다(이 시험이 그렇게 한 번 멈췄다).
    ...
    // **이 시험이 재는 것은 응답 묶음이지 이탈이 아니므로** 그것이 정확한 설정이다.
    std::fs::write(
        계획_문서(&root),
        "---\nbaseline: HEAD\n---\n# 이 저장소의 계획\n무엇을 왜\n\n## 하나\n`뿌리` 를 고친다\n",
    )
...
#[test]
fn 이관한_것이_어디로_갔는지_함께_실린다() {
    let repo = 큰_저장소("fold");
    let 이름들 = 질의_이름들();
    assert!(이름들.len() >= 최소_능력, "질의가 {}개다 — 하한 미만", 이름들.len());

    let (mut 이관한_회차, mut 안_이관한_회차) = (0usize, 0usize);
    for name in &이름들 {
        let v = 질의(&repo, name, &[]);
        ...
        assert_eq!(folded.len(), 1, "`{name}` 의 이관한 자리가 하나가 아니다");
        let f = &folded[0];
        assert_eq!(f["what"].as_str(), Some("ledger"));
        assert!(f["count"].is_number(), "이관 건수가 없다 — 크기를 알 수 없다");
    ...
    // **둘이 서로를 막는다.** 늘 이관하면 둘째가, 아무것도 안 이관하면 첫째가 실패한다.
    assert!(이관한_회차 >= 1, "이관한 회차가 없다 — 이 시험은 아무것도 안 쟀다");
    assert_eq!(안_이관한_회차, 1, "안 이관한 회차가 하나가 아니다");
...
#[test]
fn 능력_목록은_안_뭉개진다() {
    // `[f06.2.pass]` ② — 문서 §4.3 이 못 박은 자리다. 이관하면 소비자가 공백을
    // *"이상 없음"* 으로 읽는다.
    let repo = 큰_저장소("caps");
    for name in 질의_이름들() {
...
fn 이관과_생략이_같은_사건을_두_번_세지_않는다() {
    ...
    // **하한** — 생략이 안 일어나면 아래가 공짜로 통과한다.
    let 잘린 = 좁게["elision"]["truncated"].as_array().expect("truncated");
    assert!(!잘린.is_empty(), "`--node-max 2` 로도 생략이 안 일어났다 — 이 시험은 아무것도 안 쟀다");
    assert!(넉넉["elision"]["truncated"].as_array().expect("truncated").is_empty());

    // **생략이 일어나도 이관 건수는 안 변한다.** 변하면 둘이 같은 사건을 세고 있다.
    assert_eq!(
        넉넉["fold"]["folded"], 좁게["fold"]["folded"],
        "생략이 이관을 바꿨다 — 둘이 갈려 있지 않다"
    );
...
#[test]
fn 응답묶음을_지는_표면_전부가_아홉을_진다() {
    // `[f06].the_whole_surface_carries_an_envelope` — F05 가 자기 표면까지 졌고
    // **나머지가 여기다.**
    let repo = 큰_저장소("surface");

    // 응답 묶음을 지는 표면 — 질의 여섯 + `pal touch` + `pal doctor`.
    let mut 응답묶음들: Vec<(String, serde_json::Value)> = Vec::new();
    for name in 질의_이름들() {
        let v = 질의(&repo, &name, &[]);
        응답묶음들.push((format!("query {name}"), v));
    }
    for (이름, args) in [
        ("touch", vec!["touch", "뿌리", "--json"]),
        ("doctor", vec!["doctor", "--json"]),
    ] {
        let v: serde_json::Value = serde_json::from_str(&pal(&repo, &args)).expect("응답 묶음 JSON");
        응답묶음들.push((이름.to_owned(), v));
    }

    // **하한** — `--json` 을 산출하는 응답 묶음 표면이 6 개 미만이면 멈춘다.
    assert!(응답묶음들.len() >= 6, "응답 묶음을 지는 표면이 {}개다", 응답묶음들.len());
    ...
    let 남은 = 응답묶음들.iter().filter(|(_, v)| v["log"]["status"] == "recorded").count();
    let 안_남은 = 응답묶음들.len() - 남은;
...
#[test]
fn 응답묶음을_안_지는_표면은_목록으로_서고_응답묶음인_척하지_않는다() {
    // `[f06].the_whole_surface_carries_an_envelope` — *"전부가 응답 묶음을 지는 것이 아니다.
    ...
    // **응답 묶음을 지는 표면 넷.** 이름이 아니라 산출로 확인한다.
    for (이름, args) in [
        ...
        let v: serde_json::Value = serde_json::from_str(&pal(&repo, &args)).expect("응답 묶음 JSON");
        for 성분 in 응답묶음의_아홉 {
            assert!(v.get(성분).is_some(), "`{이름}` 이 응답 묶음을 진다고 했는데 `{성분}` 이 없다");
        }
    }
    ...
    // **응답 묶음을 안 지는 표면 넷 — 그리고 안 지는 것이 옳다.**
    //
    // ⚠ `query --list` 가 이 목록에서 가장 무겁다. 응답 묶음은 `Snapshot` 을 요구하고,
    ...
    for (이름, args) in &안_지는 {
        ...
        let 있는 = 응답묶음의_아홉.iter().filter(|f| v.get(**f).is_some()).count();
        // **응답 묶음인 척하면 안 된다.** 아홉을 전부 지면 그것은 응답 묶음이고, 그러면 이
        // 목록이 낡은 것이다.
        assert!(
            있는 < 응답묶음의_아홉.len(),
            "`{이름}` 이 응답 묶음의 아홉을 전부 진다 — 게이트의 목록을 고쳐야 한다"
        );
    }

    // **하한** — `--json` 을 산출하는 표면이 6 개 미만이면 이 시험이 아무것도 안 잰다.
    assert!(4 + 안_지는.len() >= 6);
