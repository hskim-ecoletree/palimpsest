...
fn 파이썬() -> &'static str {
    ...
    panic!("파이썬 실행자를 못 찾았다 — `python3` 도 `python` 도 성립하지 않는다");
}
...
/// **음성 대조 둘째** — 빈 커밋 범위에서 칸이 **사라지지 않고 「못 셌다」**를 산출한다.
///
...
///
/// 걷기만 하는 것도 안 된다: 그러면 ③④⑤⑥ 이 **거짓 0** 을 산출한다. 그래서 **양방향**으로
...
fn 계기판이_빈_범위에서_칸을_안_삼킨다() {
    ...
    for 칸 in ["③ 진자 (P1)     — **못 셌다**", "④ 연쇄 깊이      — **못 셌다**"] {
        assert!(빈.contains(칸), "빈 범위에서 0 을 산출했다 — 「못 셌다」여야 한다:\n{빈}");
    }
    ...
    // ② 파서 — 펜스 안과 `## 범위 밖` 을 안 세고, 들여쓴 상자는 잰다.
    assert!(
        빈.contains("② 미판정 잔액    2 / 3"),
        "조건 셋(닫힘 1·열림 2)이어야 한다 — 펜스·범위 밖을 세거나 들여쓰기를 놓쳤다:\n{빈}"
    );

    // ③ 안 빈 범위 — 같은 칸이 **수**를 산출한다. 한 방향만 재면 「언제나 못 셌다」를 못 가른다.
    let git = |args: &[&str]| {
