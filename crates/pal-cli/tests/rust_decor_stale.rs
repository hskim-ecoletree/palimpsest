...
//! 같았고 **그 변경에 결박이 `fresh` 로 남았다** — 사실이 아닌 것을 사실로 적었다.
//!
//! # 갈래마다 따로 잰다
//!
...
//!
//! # 음성 대조 — 속성 안의 주석·공백은 낡음을 안 붙인다 (ADR-0007)
//!
//! 속성을 **원문 바이트**로 요약하면 위 다섯이 다 잡히고 포매터 한 번에 결박 무더기에
//! 낡음이 붙는다(R-07). 그 반대 방향을 같은 픽스처가 잰다.
...
fn 갈래(tag: &str, 조각: &str, from: &str, to: &str) {
    ...
    assert_eq!(
        신선도(&repo, 조각),
        "stale",
        "★ `{from}` → `{to}` 를 커밋했는데 결박이 stale 이 아니다 — 사실이 아닌 것을 사실로 적는다"
    );
...
fn a2a_1_속성_안의_주석과_공백만_바꾸면_fresh() {
    ...
    assert_eq!(신선도(&repo, 구조체_조각), "fresh", "★ 속성 안의 주석·공백만 바꿨는데 낡음이 붙었다 (R-07)");
    assert_eq!(신선도(&repo, 메서드_조각), "fresh", "★ 수신자의 공백만 바꿨는데 낡음이 붙었다 (R-07)");
    let _ = std::fs::remove_dir_all(&repo);
}
