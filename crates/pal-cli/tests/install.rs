...
/// (c) 이미 깔린 곳 — **두 번째 설치가 첫 번째와 같은 상태를 산출한다.**
#[test]
fn 두_번째_설치가_같은_상태를_산출한다() {
...
fn 갱신_뒤에_진단이_빨개지지_않는다() {
    ...
    // ② 그런데 **진짜 고장은 여전히 빨갛다** — 이 줄이 없으면 위가 공짜로 통과한다.
...
/// 산출한다. 없으면 어떤 질의 형태로도 **패턴을 하나도 못 만든다**:
///
/// ```text
...
// 남아 `모드가_살아_있다` 로. 둘 다 아래에 있다. 하나로 묶여 있던 동안은 **이식
...
/// ★ **파일시스템이 답을 다르게 산출하는 자리에서 우리는 같은 답을 돌려준다** — `§3-C ①`.
///
/// # 이것이 다른 플랫폼 분기와 다른 점
///
/// 이 저장소의 다른 분기는 전부 **API 의 차이**였다(모드 비트·확장자·링크 수).
/// 여기는 **같은 코드가 같은 호출을 하는데 파일시스템이 다른 답을 돌려준다**:
///
...
/// 곳에 따라 다르게 잡힌다. 그래서 [`install::casing`] 이 **양쪽에서 멈춘다.**
///
...
/// | | 365자 경로에서 |
/// |---|---|
/// | `std::fs::create_dir_all`·`read`·`write` | **성립한다.** Rust std 가 절대 경로에 `\\?\` 를 붙인다 |
/// | `pal install --target <긴 경로>` | **rc=0.** 라이프사이클이 돈다 |
/// | `Command::current_dir(<긴 경로>)` | **성립하지 않는다** — `Os { code: 267, NotADirectory }` |
/// | `git -C <긴 경로> init` | **성립하지 않는다** — `fatal: … Filename too long` (`core.longpaths=true` 를 줘도 같다) |
///
...
///
/// 실측(2026-08-17)이 그 사이의 구멍을 산출했다 — `install/hooks.rs` 의 아홉 자리가
...
/// (*"우리가 산출하는 경로 문자열은 한 함수를 지난다"*)은 세 플랫폼에 다 있는 문장이다.
#[test]
...
/// `.gitignore` 에도 살기 때문이다). 그러니 git 이 없으면 그 걸음이 **원리상 성립하지 않는다.**
///
...
fn git_이_없으면_1단계에서_멈춘다() {
    ...
    assert!(!out.status.success(), "git 이 없는데 rc=0 을 돌려줬다");
...
/// ★ **경로가 유효한 UTF-8 이 아니어도 라이프사이클이 성립한다** — `§3-C ③`.
///
/// # 무엇이 위험이었나
///
...
/// 자리가 없다.
#[test]
fn 유효한_utf8_이_아닌_경로에서도_라이프사이클이_성립한다() {
    let base = 방("g-비utf8");
    let 이름 = 비utf8_이름();
...
fn 쓰기_불가_디렉터리에서_거짓_성공하지_않는다() {
    ...
    assert!(!out.status.success(), "쓰기 불가 디렉터리에서 rc=0 을 돌려줬다");
...
///
/// `.claude/agents/` 는 **남의 에이전트가 함께 쓰는 자리**라 매니페스트가 그쪽만
...
/// 검사할 수 없는 것은 **`Residual`** 로 산출한다 — `pal doctor` 가 이미 쓰는 어휘다.
#[test]
fn 설치가_없으면_잔여로_산출한다() {
...
// fixture 는 저쪽에 있다 — 같은 성질을 두 곳에서 세우면 그것이 곧 drift 다(진행 규칙 4).
...
/// 를, `uninstall` 은 rc=1 을 산출했다 — 사용자에게 남는 길이 손으로 지우는 것뿐이었다.
#[test]
