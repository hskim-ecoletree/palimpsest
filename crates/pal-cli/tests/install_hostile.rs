...
/// ★ **볼 수 있는 플랫폼에서는 끊었다고 말한다.**
///
/// 결과는 두 플랫폼이 같지만 **서술은 다르다** — 유닉스는 링크 수를 셀 수 있으므로
/// 그 사실을 화면에 내고, Windows 는 못 세므로 못 낸다. 그 차이는 우리가 고른 것이
/// 아니라 **보이는 것의 차이**이고, 볼 수 있는 곳에서 입을 다무는 것은 대칭이 아니라
/// 정보를 버리는 것이다.
///
/// 짝은 아래 `#[cfg(not(unix))]` 이 진다 — **말하지 않는다는 사실 자체**를 시끄럽게 둔다.
#[test]
#[cfg(unix)]
fn 끊었다고_말한다() {
    let 방 = 방("하드-알림");
...
/// ★ **이 플랫폼은 끊지만 말하지는 못한다 — 그 사실이 시끄러워야 한다.**
///
/// ⚠ **방어가 없다는 뜻이 아니다.** `설치가_하드링크를_끊고_밖을_안_고친다` 가 이
/// 플랫폼에서도 돌고 통과한다. 여기서 외치는 것은 **서술의 한 겹**이다 —
/// 링크 수를 셀 수 없으므로(`number_of_links()` 는 unstable, raw FFI 는 `unsafe 금지`
/// 게이트가 막는다) *"끊었다"* 를 화면에 낼 근거가 없다.
#[test]
#[cfg(not(unix))]
fn 끊었다는_말이_이_플랫폼에서는_안_나온다() {
    panic!(
        "하드링크를 끊었다는 **말**을 이 플랫폼에서는 못 한다 — 링크 수를 셀 수 없어서 \
         (`number_of_links()` 는 unstable, raw FFI 는 `unsafe 금지` 게이트가 막는다) \
         언제 끊었는지 모른다.\n    \
         ★ **방어는 선다** — `설치가_하드링크를_끊고_밖을_안_고친다` 가 이 플랫폼에서도 \
         돌고 통과한다. 여기서 없는 것은 방어가 아니라 **서술**이다"
    );
}
