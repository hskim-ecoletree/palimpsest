...
// 곧 *"두 플랫폼이 같은 답을 돌려준다"* 의 증거다 — 시험 파일이 그 사실을 진다.
//
...
/// 한 줄로 출력한다(`install::쓸_수_있나`). 문구는 여전히 다르지만 — 셀 수 있는 쪽은
...
/// 없거나"* 라는 규율은 **우리 대신 읽는 프로세스에도** 걸린다.
#[test]
#[cfg(unix)]
...
/// **지금 매니페스트가 적은 항목 수** — 베끼지 말고 헤아린다.
///
/// ★ 앞 판은 시험이 *"설치가 적는 경로가 17개다"* 를 **주석으로 베껴 놓고** 230 을
/// 채웠다. 그 수는 놓은 날부터 갈렸다 — `PAYLOAD` 가 늘자 **실측 20** 이 되어
/// `230 + 20 + …` 이 상한 256 을 넘었고 **CI 세 OS 가 전부 빨개졌다**
/// (2026-08-19 · 실측 259). **검사의 모집단은 검사가 안다. 베끼지 말고 돌려라.**
fn 지금_항목_수(root: &Path) -> usize {
...
/// **상한 바로 아래까지 채운다** — 여유 `틈` 만 남기고.
fn 상한_바로_아래까지_채운다(root: &Path, 틈: usize) -> usize {
    let 지금 = 지금_항목_수(root);
    let 목표 = pal_cli_항목_상한() - 틈;
    assert!(지금 < 목표, "설치만으로 이미 상한 가까이다 — 지금 {지금} · 목표 {목표}");
    항목을_불린다(root, 목표 - 지금);
    목표
}

/// 상한 상수 — **시험이 손으로 적지 않는다.**
const fn pal_cli_항목_상한() -> usize {
    256
}
...
fn 상한_안의_큰_매니페스트도_빠르게_걷힌다() {
    let 방 = 방("상한가장자리");
    성공(&방.안, &["install"]);
    // ★ **베끼지 말고 헤아린다.** 설치가 적는 수는 `PAYLOAD` 가 늘 때마다 바뀐다 —
    //    앞 판은 「17개」를 주석으로 베끼고 230 을 채웠다가 상한을 넘겼다.
    let 채운수 = 상한_바로_아래까지_채운다(&방.안, 6);
    assert!(채운수 < pal_cli_항목_상한(), "상한 안이어야 한다");
