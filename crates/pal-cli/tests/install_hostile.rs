...
/// ★ **Windows — 셀 수 없으니 끊고 쓴다. 그래도 밖은 안 바뀐다.**
///
/// 앞 회차는 여기에 *"이 플랫폼에서는 안 재진다"* 외침만 있었다. 그 외침은 제 일을
/// 했다 — 실측이 **rc=0 으로 밖의 파일에 우리 블록이 실리는 것**을 잡아냈다. 이제
/// `guard::제자리를_준비한다` 가 그 자리를 막았으므로 **외침을 시험으로 승격한다.**
///
/// ⚠ **유닉스와 단언이 다르다. 그것이 이 시험의 내용이다.**
///
/// | | 설치 | 밖의 파일 | 왜 |
/// |---|---|---|---|
/// | 유닉스 | **실패(rc≠0)** | 그대로 | 링크 수를 **볼 수 있어서** 사람에게 넘긴다 |
/// | Windows | **성공(rc=0)** | 그대로 | 볼 수 없어서 **끊고 쓴다** — 유닉스가 사람에게 시키는 그 처방을 쓰기 경로가 대신 한다 |
///
/// **양쪽이 같이 지는 것**: `[f24]` ⑦ — 밖의 바이트가 한 개도 안 바뀐다.
/// 그 한 줄이 여기서 재는 전부이고, 나머지는 그것에 이르는 길이 다를 뿐이다.
#[test]
#[cfg(windows)]
...
/// ★ **유닉스도 Windows 도 아닌 자리에서는 아직 안 재진다 — 그 사실이 시끄러워야 한다.**
///
/// 짝 없는 `#[cfg]` 시험은 다른 플랫폼에서 **조용히 사라지고**, 그러면 경계 방어가
/// 사라진 줄도 모른다. 그래서 짝을 남겨 둔다.
#[test]
#[cfg(not(any(unix, windows)))]
fn 하드링크_방어가_이_플랫폼에서는_안_재진다() {
    panic!(
        "하드링크로 대상 밖이 새는지를 이 플랫폼에서 아직 안 잰다 — \
         `install/guard.rs` 는 유닉스에서 **세고** Windows 에서 **끊는다.** \
         이 플랫폼에는 둘 중 어느 것도 없다"
    );
}
