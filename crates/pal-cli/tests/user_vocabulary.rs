...
const 금지_패턴: &[(&str, &str)] = &[
    ...
    // ⚠ **숫자 둘 다음 글자가 16진수면 기능 절이 아니다.** 앞 판(`\[f[0-9]{2}`)은 승인 화면이 싣는
    // **결박 ID**(16 자리 16진수)가 우연히 `[f48cc959b0498ed9]` 처럼 `f`+숫자 둘로 시작하면 걸렸다 — ID 가
    // 실행마다 달라 CI 가 한 번씩만 빨갰다(2026-09-14 · 런 34859001054 ubuntu). `[f11 는` · `[f11는` · `[f24]` ·
    // `[f05.3.pass]` 는 그대로 걸린다(`d1a_심은_문자열은_…` 이 진다). 대가: 소문자 접미사가 붙은 `[f11a]` 는 못 잡는다.
    // `regex` 크레이트에 앞보기가 없어 「16진수가 아닌 글자 또는 끝」으로 적는다.
    ("기능 절", r"\[f[0-9]{2}(?:[^0-9a-f]|$)"),
    ("ADR", r"ADR-[0-9]{4}"),
    ("문서 절", r"§"),
];
...
fn d1a_제품_낱말과_해시와_등급은_안_걸린다() {
    ...
    let 화면 = "회차를 연다 · L2 exact · L0 unavailable · UTF-8 · repo@aded7ce+worktree#335e5bc23a81 · body eace1ea25a2d · p95 · 결박    [f48cc959b0498ed9] · [f0123456789abcde]";
    let 걸림 = 걸린_것(화면, &[]);
    assert!(걸림.is_empty(), "걸리면 안 되는 것이 걸렸다: {걸림:?}");
    // 음성 대조 — 좁힌 「기능 절」 이 기능 절 표기는 여전히 잡는다.
    for 표기 in ["[f24]", "[f05.3.pass]"] {
        let 걸림 = 걸린_것(표기, &[]);
        assert!(걸림.iter().any(|(이름, _, _)| *이름 == "기능 절"), "`{표기}` 를 기능 절로 못 잡는다: {걸림:?}");
    }
}
