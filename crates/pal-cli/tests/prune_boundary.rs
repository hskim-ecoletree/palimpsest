...
fn prune_은_cache_밖의_바이트를_하나도_안_바꾼다() {
    ...
    // ② **예산 0 · 그리고 F05 가 더한 손잡이 둘도 함께 민다.**
    //
    // ★ `[f05.5.pass]` ③ — 새 손잡이가 지우는 범위를 넓히면 그것이 R-21 의 재발이다.
    // **안 부르면 공짜로 통과하므로 부른 회차에서 잰다.** 지울 수 있는 것을 전부
    // 지우라는 뜻이고, 경계를 가장 세게 미는 회차다.
    let out = pal(&repo, &[
        "cache", "prune", "--repo", ".", "--budget", "0",
        "--sweep-quarantine", "0", "--sweep-stray", "--stray-age", "0",
    ]);
