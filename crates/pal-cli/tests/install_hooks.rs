...
//! | 그 실행이 **차단 바이트를 산출하는가** | 표준출력이 `{"decision":"block","reason":…}` · 종료 코드 0 |
//! | **두 번 설치해도 한 번만 도는가** | 등록이 하나여야 한다 |
//! | 남의 등록을 **안 건드리는가** | 사용자 훅이 든 fixture 에서 왕복 후 값 비교 |
//! | **옛 형태를 새 형태로 옮기는가** | 손으로 shell form 을 심고 `update` 뒤에 남은 것을 헤아린다 |
...
//! 게이트 문서([F24-크로스플랫폼](../../../docs/gates/F24-cross-platform.md))에 있다 —
...
//! 증거이기도 하다.
//!
//! **그래서 이 파일이 지는 것은 바뀌지 않는다** — 여기는 *"우리가 산출하는 바이트가 실측된
//! 규약과 같은가"* 를 **매 커밋마다** 잰다. 그 바이트가 하네스에서 무슨 일을 하는지는
//! 게이트가 한 번 재고 문서로 남는다. 둘은 주기가 다르고, 그래서 있는 자리도 다르다.
//!
...
/// 제약이고, macOS 는 `ETXTBSY` 를 이 형태로 내놓지 않는다. **그래서 리눅스에서만 났다.**
///
...
fn 하네스처럼(exe: &Path, 항목: &serde_json::Value, payload: &str) -> Output {
    ...
    // 사용자 환경의 `PATH` 로 그 이름을 푼다. 그래서 이 fixture 도 pal 이 있는 자리를
...
fn 등록된_명령이_돌고_차단을_산출한다() {
    ...
    assert!(out.status.success(), "등록된 명령이 exit 0 을 안 산출했다");
    assert!(
...
fn 실행_파일이_옮겨_가도_등록이_안_흔들린다() {
    ...
    // **다른 자리의 pal** 로 갱신한다 — 다른 사람의 기계를 흉내내는 자리다.
...
///
/// 파일이 사라지거나 실행 권한을 잃으면 `/bin/sh` 가 exit 127·126 을 산출하는데, 하네스는
...
fn 진단이_남이_심은_항목을_안_돌린다() {
    ...
    let c = 훅_검사(&exe, &root);
    assert_eq!(c["outcome"], "failed", "남이 심은 항목을 초록으로 산출했다: {c}");
...
#[test]
fn 진단이_대상_밖_실행_파일을_초록으로_안_산출한다() {
    ...
    assert_eq!(c["outcome"], "failed", "대상 밖 실행 파일을 초록으로 산출했다: {c}");
    let detail = c["detail"].as_str().expect("detail");
    assert!(detail.contains("update"), "무엇을 하라고 안 적었다: {detail}");
