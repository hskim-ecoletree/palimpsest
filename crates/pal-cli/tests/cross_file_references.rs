...
/// `A3` 의 절반 — 잠근 축1 의 **①(균일 경로 형제 모듈)** 과 **④(`crate::`·`super::`
/// 접두)**. 나머지 둘(**②** 형제 크레이트 깊은 경로 · **③** 형제 크레이트 평평 경로)은
/// [`a3_형제_크레이트_평평_경로와_깊은_경로가_각각_엣지가_된다`] 가 진다.
///
/// ⚠ **한 시험이 넷을 다 재게 못 한다** — ②③ 은 크레이트가 둘 있어야 하고 이 픽스처에는
/// 크레이트가 하나도 없다(`crates_of` 는 `<이름>/src/lib.rs` 꼴만 뿌리로 인정한다).
/// 그래서 **이름을 재는 것에 맞췄다** — 앞 이름은 `…넷이_각각…` 이었고, 그 이름으로
/// 하나만 재던 것이 `DL4-17` 이 잡은 자리다.
#[test]
fn a3_균일_경로와_접두가_각각_엣지가_된다() {
    let repo = 저장소("a3");
    pal(&repo, &["touch", "씀", "--json"]);
    let p = 투영(&repo);
    let 대상 = 파일_간_대상(&p, 씀(&p), "src/user.rs");

    // ① 균일 경로 형제 모듈 · 같은 크레이트 안의 깊은 경로 · 별칭이 걸린 형제 모듈
    //   ⚠ 「깊은 경로」는 **같은 크레이트 안**이라 잠근 축1 의 ② 가 아니다 — ② 는
    //     형제 크레이트를 요구하고 아래 다른 시험이 진다.
    for (형태, 이름) in [
        ("균일 경로 형제 모듈", "Root"),
        ("같은 크레이트 안의 깊은 경로", "Leaf"),
...
fn e4_화면이_능력_축_셋을_가르고_회계를_찍는다() {
    ...
    // ★ **옮긴 단언이다 — 지운 것이 아니다** (2026-09-13 · 회차 `first-release-elsewhere` 의
    //   `D2`). 앞 판은 문구 「범위 밖」을 찾았고, 그 말이 이 저장소의 회차 어휘라 화면에서
    //   빠졌다. 지키던 구별(능력 부재 ↔ 안 하기로 정함 ↔ 원리상 못 함)은 **등록한 두
    //   문자열**로 남는다.
    assert!(
        화면.contains("안 하기로 정한 자리입니다"),
        "`x.foo()` 를 「능력 부재」로 적으면 사람이 「곧 될 것」으로 읽는다:\n{화면}"
    );
    assert!(
        화면.contains("원리상 풀 수 없는 자리입니다"),
        "「원리상 못 함」 갈래가 화면에서 사라졌다 — 안 하기로 정한 것과 섞인다:\n{화면}"
    );
    assert!(
        화면.contains("호출자 수는 하한입니다"),
        "호출자 수가 「깨지는 곳 전부」로 읽힌다 — 최상위 참조가 빠진다는 말이 없다:\n{화면}"
    );
    assert!(
    ...
    assert!(
        화면.contains("못 선 까닭의 성격"),
        "이관표가 화면에 없다 — 못 푼 것이 결함인지 경계인지 사람이 못 읽는다:\n{화면}"
    );
    for 문 in ["재수출을 지나는 이름", "심볼로 안 서는 이름", "고르지 않습니다"] {
