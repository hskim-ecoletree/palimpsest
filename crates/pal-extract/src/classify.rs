...
use pal_core::MARKER_SCAN_BYTES;
...
// **크기 상한과 표식 스캔 길이는 여기 없다.** `pal-core::budget` 한 곳이다
// (stack §5.5 · `[f05.1.pass]` ①). 재수출도 남기지 않는다.
...
pub fn classify(
    ...
    // ⑥ 임계 강등 — **`partial` 이 문법 부재를 가리지 않게** (옛 F02 §3.4 · `[f02.2.pass]` ④)
    //
...
pub const fn grade_of(language: pal_core::Language) -> ExtractGrade {
    match language {
        ...
        // Rust 추출기 — **중첩 순회 + 스코프 체인. 그런데 등급은 L1 이다.**
        // (#66 · #130 · 소유자 지시 2026-08-20 §2 · 2026-09-07)
        //
        // ★★ **2026-09-08 부터 이 값은 「스코프가 없다」는 뜻이 아니다.**
        // 회차 `2026-09-07-rust-scope-references` 가 `ScopeChain` 을 채워 REFERENCES
        // 엣지를 내면서 **등급 글자는 그대로 두기로** 골랐다(소유자 답: *"엣지만 · L1
        // 유지"*). 그러므로 아래 둘이 갈린다:
        //
        // | 무엇 | 값 |
        // |---|---|
        // | 이 함수가 돌려주는 언어 등급 | `L1` |
        // | 실제로 하는 일 | 스코프 해소된 파일 안 참조 |
        //
        // # 왜 안 올렸나 — **되돌리기 비용이 비대칭이다**
        //
        // `L2` 로 올리면 `identity()` 가 `Exact` 가 되고, 그러면 `body_digest` 가 지역
        // 이름을 지우기 시작해 **Rust 심볼의 요약이 전부 이동한다.** 그 순간 이 저장소의
        // 결박 25 건이 통째로 `stale` 이 된다 — 그것은 이 회차가 답하려는 물음(*"여기를
        // 바꾸면 무엇이 깨지나"*)과 **다른 축**이고, 한 회차에 둘을 겹치면 어느 쪽이
        // 결박을 움직였는지 못 가른다.
        //
        // # 언제 풀리나 — **#133 이 진다**
        //
        // 승격 조건은 셋이다: ① Rust 의 shadowing 이 「가장 뒤」 팔로 정확히 해소되는
        // 것이 표본으로 확인되고 ② `Namespace` 가 매크로 이름 공간을 담고
        // ③ 결박 재작성 절차가 서면. 그때 `L2` 로 올리고 캐시와 결박을 함께 옮긴다.
        //
        // ⚠ **`ResolutionGrade::Scoped` 와의 어긋남은 없앴다** — `schema/graph.toml`
        // 의 `[edge.REFERENCES]` 가 「L2 이상에서만」이 아니라 **「스코프 체인이 선
        // 언어에서만」**으로 다시 적혔다(`C9`). 등급 글자가 아니라 `FileGraph::scopes`
        // 가 그 자격을 진다.
        //
        // **L1 이 여전히 지는 것**: Rust 심볼의 `identity` 가 전부 `Ordinal` 이라
        // **선언 순서에 취약한 것과 그냥 L1 인 것이 대장에서 같은 글자**가 된다.
        // 그 갈림은 #66 의 분할이 지고, **세는 자리는 `--example coord_collisions` 다**
        // (수를 여기 안 적는다 — 앞 판이 적은 6.1% 는 재현되지 않았다 · 독립 리뷰 R3).
