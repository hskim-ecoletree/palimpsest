...
//!
//! # ⚠ 이 캐시의 **부피 상한은 아직 아무도 안 잰다** (2026-09-08 · #130)
//!
//! Rust 가 스코프 체인을 싣기 시작하면서 항목 하나가 파일마다 수백~수천 개의
//! `LocalRef` 를 담는다(이 저장소 실측: 참조 6,639 · 138 파일). **그 비용을 재는 자리가
//! 없다** — `budget.rs` 에 항목 크기 축이 없고, 그것을 만드는 것은 이 회차가 안 만들기로
//! 한 장치다. **범위 밖으로 수용한 것이지 없는 문제가 아니다.**
//!
