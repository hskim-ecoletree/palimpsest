...
pub struct RustScopeRules {
    // ★★ **수를 여기 안 적는다** (2026-09-10 · `BL3-01`).
    //
    // 앞 판은 규칙마다 *"끄면 실측 가짜 엣지 52"* 처럼 **잰 값을 주석에 캐시**했다.
    // 그 수는 코퍼스가 자라면 즉시 낡는데 아무 검사도 그것을 안 잰다 —
    // 실측(2026-09-10)으로 열 줄 중 여럿이 갈렸다: `V2` 는 58 이라 적혀 있고 계기는
    // **94** 이고, `V5` 는 60 이라 적혀 있고 계기는 **13** 이다.
    //
    // ⚠ **그러므로 새 수로 갈아 끼우지 않는다** — 그러면 같은 자리에 같은 병을 다시
    // 심는 것이다. 이 회차가 이미 같은 형태를 한 번 고쳤다(`no_symbol` 의 「340 중
    // 298」을 회계 열쇠로 올렸다). **수가 서는 자리는 계기 하나다:**
    //
    // ```text
    // cargo run -q -p pal-extract --example scope_variants
    // ```
    //
    // 그 계기가 규칙마다 **엣지 · 가짜 · 누락**과 「엣지 축이 갈리나」를 찍고, 안 갈리는
    // 규칙에 `✗ 이 규칙은 등록된 축에서 아무것도 안 잰다` 를 붙인다. 아래 주석이 지는
    // 것은 **그 규칙이 무엇을 하는가**뿐이다.
    /// `V1` — 파라미터가 이름을 선언하나.
    pub 파라미터_선언: bool,
    /// `V2` — `let`·`match` 팔·`for`·`if let` 의 패턴이 이름을 선언하나.
    pub 패턴_선언: bool,
    /// `V3` — `impl`·`trait` 본문이 스코프를 여나.
    pub impl_이_연다: bool,
    /// `V4` — 아이템 선언을 `hoist_home` 으로 옮기나. **켜면 `V3` 를 정확히 상쇄한다.**
    pub 아이템을_hoist_home_에: bool,
    /// `V5` — `#[…]` 안의 이름을 배제하나(`#[cfg(test)]` 의 `test` 가 그 형태다).
    pub 속성_배제: bool,
    /// `V6` — 매크로 `token_tree` 안에서 앞 형제가 `.`·`::` 이면 배제하나.
    pub 매크로_앞형제_거르기: bool,
    /// `V7` — `a::b` 의 `b`(경로 꼬리)를 배제하나.
    ///
    /// ★ **끄는 것이 축2 의 ⓑ 를 세우는 길이 아니다** — 회차
    /// `2026-09-08-cross-file-references` 의 `A7` 이 그 등식을 반증했고, 세우는 길은
    /// **(다2)** 다: 이 규칙을 켠 채로 임포트된 머리의 꼬리를 2 층으로 실어 **대상
    /// 파일에서만** 찾는다(`pal_core::cross_file`).
    pub 경로_꼬리_배제: bool,
    /// `V8` — `x.foo` 의 `foo`(`field_identifier`)를 배제하나.
    pub 필드_식별자_배제: bool,
    /// `V9` — `use` 절 안의 이름을 참조에서 배제하나.
    pub use_절_배제: bool,
    /// `V10` — `cfg` 쌍둥이를 조용히 하나로 고르나.
    pub cfg_쌍둥이_해소: bool,
}
...
impl ScopeRules for RustScopeRules {
    ...
    /// 서 버리기 때문이다. 그래도 끄지 않는다: 이 훅은 기준 조합에서만 값을 만든다.
