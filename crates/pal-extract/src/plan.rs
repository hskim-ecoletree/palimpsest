//! 계획 문서 인입 — **조각화는 [옛 F10] 의 것을 그대로 쓰고, 좌표 추출만 F12 가 세운다**.
//!
//! # 왜 [옛 F10] 의 신호 규칙을 그대로 못 쓰는가 — **실측이 그것을 막았다**
//!
...
//! # 여기에 판단이 없다
//!
//! 이 모듈이 산출하는 것은 **패턴**이지 좌표가 아니다. *"이 이름이 기준선에 서는가"* 는
//! [`pal_core::resolve_plan`] 가 묻고, 그것이 [ADR-0019] 의 자격 검사다.
//! 조각화가 2 층을 안 타는 것과 같은 갈림이다.
//!
//! [옛 F10]: ../../../docs/plan/disposal-map.md
...
/// 계획이 좌표를 **명시**하는 줄의 머리 — [옛 F12 §4] 의 *"좌표를 요구하는 템플릿"*.
...
///
/// 옛 `F12 §3.4` 가 *"`docs/plan/features/*.md` 의 **완료 체크리스트 항목들**이 첫 `Plan`"*
...
/// 프론트매터의 `baseline:` — **[옛 F12 §4] 가 요구한 「계획에 기록된 Snapshot」이다.**
///
/// ⚠ `--base <ref>` 를 안 만든다. 그 손잡이의 소유자는 [옛 F23 §7] 이다.
///
/// # 왜 YAML 파서를 안 들이나
///
...
/// 경로처럼 생겼으면 패턴으로.
///
/// **[옛 F10] 의 판정을 그대로 쓰되 하나를 더한다** — `src/order/**` 처럼 **확장자가 없고
/// `*` 가 있는 것**도 받는다. [옛 F12 §3.1] 이 세 형태 중 하나로 그것을 이름으로 적었고,
...
mod tests {
    ...
    fn 체크박스가_있으면_체크박스가_항목이다() {
        // 옛 `F12 §3.4` — *"완료 체크리스트 항목들이 첫 `Plan` 이다"*. 그 문서는
        // 2026-08-18 에 지웠다(`docs/plan/disposal-map.md`).
        let p = 계획(
            "# 머리\n설명\n\n## 완료 체크리스트\n\n- [ ] `Plan` 타입\n- [x] `Deviation` 타입\n",
        );
