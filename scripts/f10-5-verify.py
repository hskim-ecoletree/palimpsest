...
# 이 스크립트가 막는 「대조가 꺼지는 형태」 넷
  ...
  · ★ **열아홉째(한 대조의 개선이 다른 대조의 하한을 끈다)** — 이 이슈에 **직접** 걸린다.
    ...
    **거리 있는 신호가 유일해도 확정 안 하는지**를 함께 잰다
  · **첫째(변형이 아무것도 안 바꿈)** — 신호를 빼는 것은 **코드 변경**이다. ③이
    **뺀 신호의 날것이 실려 있는데도 후보를 하나도 안 산출하는지**를 직접 잰다.
...
def 해소(tmp: Path, tag: str, src: Path, pin: str) -> dict:
    ...
    #   ⚠ **날것이 0 이면 이 검사가 아무것도 안 잰다** — 그것도 잡는다.
...
def 표본(전체: dict[str, dict]) -> None:
    ...
    if 잘못:
        fail("① 거리 있는 신호가 확정했다",
             f"{잘못} 가 `결박됨` 을 산출했다 — **처분이 코드에 안 박혔다. 구현의 반증이다**")
    elif 걸린:
...
def 동점(tmp: Path) -> None:
    ...
    doc.write_text(
        ...
        f"---\ngrounds: [\"{동점파일}\"]\n---\n\n"
        f"# 프론트매터-동점\n\n`{동점파일}` 의 것들을 말한다.\n\n"
        # ② **확정** — 거리 0 이고 유일하다. 없으면 「전부 후보로 잡는다」가 만점을 받는다.
        f"# 확정\n\n확정할 자리다.\n\n"
    ...
    for path, 앵커, 기대분류, 기대신호 in 기대:
        ...
        if 났다 != 기대분류:
            사유 = {
                ...
                ("candidates", "unbound"): "**동점을 미결박으로 뭉갰다**",
...
def 일괄(전체: dict[str, dict]) -> None:
    ...
    if 후보있음 < BATCH_REFUSAL_MIN:
        fail("⑧ 거부 하한",
             ...
             f"**남은 거부 사유의 모집단이 0 이고, 그러면 이 하한이 멎는다**")
    else:
        ok("⑧ 거부 하한",
           f"후보 여럿인 제안 {후보있음}건 ≥ {BATCH_REFUSAL_MIN} — **남은 사유가 하한을 진다**")

    # ⚠ **도달 불가가 된 사유를 세어 둔다** — *"안 걸린다"* 와 *"없다"* 를 가른다.
    ...
    skip("⑧ 도달 불가가 된 사유",
         ...
         f"⚠ **자리는 남긴다** — 거리 있는 신호가 확정을 내게 되면 다시 걸려야 한다")
