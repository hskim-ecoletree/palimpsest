...
알고 있던 것 넷은 `[f11].what_was_already_known` 에 있다.
    ...
    ② ①을 **같은 좌표 / 다른 좌표**로 갈라 잰다 (다른 좌표 모집단 ≥ 1)
    ...
    ⑥ **질의 카탈로그** — `binding.touch` 가 네 자리에 함께 성립한다. MCP 는 **대조 불가**
...
def 최상위_선언(nodes: list[dict], path: str, 줄: set[int]) -> list[str]:
    ...
    **최상위**는 `container` 가 빈 것이다 — F03 의 정의 그대로.
    여럿이면 그대로 출력한다(규칙이 하나를 고르라고 하지 않았다). 부르는 쪽이 잰다.
    """
    맞는 = [
        n for n in nodes
...
# ═════════════════════════════════════════════════════════════════════════════
# ①② 재발 사례 재현 — **전수 다섯.** 그리고 같은 좌표 / 다른 좌표를 갈라 잰다
# ═════════════════════════════════════════════════════════════════════════════
...
def 근접(repo: Path, box: Path) -> None:
    ...
    if 고른:
        ...
        ok("③ 하나를 고르지 않는다", f"표본 {len(표본)} 전부 `unknown` 이다 — 후보만 산출한다")
    if 맞춘 >= NEAR_MIN:
...
def 반대_방향(repo: Path, box: Path, 판정: list[dict]) -> None:
    ...
    if 가장_많은 > TOP_N and 이름:
        ...
        if 실린 <= TOP_N and 잘린 >= ELISION_POPULATION_MIN:
            ok("④ 상한과 생략", f"`{이름}` 에 결박 {가장_많은} · 실린 것 {실린} · 잘린 것 {잘린}")
        else:
            fail("④ 상한과 생략", f"실린 것 {실린} (상한 {TOP_N}) · 잘린 것 {잘린}")
    elif 이름:
        ...
        skip("④ 상한과 생략", "결박된 좌표가 2층에 없다 — **대조 불가**")
...
# ═════════════════════════════════════════════════════════════════════════════
# ⑦ 지연 — **두 시계를 둘 다 산출한다. 합격선은 질의 시간에만 걸린다**
# ═════════════════════════════════════════════════════════════════════════════
...
def 지연(repo: Path, box: Path) -> None:
    ...
    # **결박이 실린 좌표의 수를 함께 싣는다** — 0 이면 조립할 것이 없어서 빠른 것이다.
