...
`[f10.6].measurement_already_seen` 에 있다.
    ...
    ④ ★ **안 붙여야 할 것을 계속 안 붙이는가** — 빈 줄로 갈린 것 · 붙을 선언이 없는 것.
      **결박 0 · 모집단 각 ≥ 1**. ⚠ **소스에서 줄로 잰다** — 산출로 세면 순환한다
...
def 해소(tmp: Path, tag: str, src: Path, pin: str) -> dict:
    ...
    # ★ **표식 주석만 따로 헤아린다** — 이 절이 고친 것이 그 경로다.
    ...
    if not 주석:
        ...
        ok(f"② {tag} 표식 주석",
           ...
           f"미결박 {주석분포['unbound']} — **셋을 기록으로 산출한다**")
...
# ═════════════════════════════════════════════════════════════════════════════
# ④ ★ 음성 대조 — **안 붙여야 할 것을 계속 안 붙이는가.** 소스에서 줄로 잰다
# ═════════════════════════════════════════════════════════════════════════════
...
def 갈래(내용: str, 시작줄: int, 본문: str) -> tuple[bool, str]:
    ...
    ⚠ **`다음_선언` 과 다른 방법으로 잰다.** 같은 방법으로 세면 구현이 자기를 채점한다.
    """
    줄들 = 내용.split("\n")
...
def 음성대조(전체: dict[str, dict]) -> None:
    print("④ ★ **안 붙여야 할 것을 계속 안 붙이는가** — 소스에서 줄로 잰다")
...
def 코틀린(tmp: Path) -> None:
    ...
    # ⚠ **실 코퍼스에 표식 주석이 몇 건인지 먼저 헤아린다** — 0 이면 그것이 [ADR-0002] 다.
...
def 재측정_모집단(전체: dict[str, dict]) -> None:
    ...
    **그러므로 이 축에서 도달 가능한 최대가 10 이다** — 결박이 얼마나 늘든.
    ...
    """
    print("⑦ 짝 — **재측정의 모집단을 잰다.** 잘라서인가 희소해서인가")
    for tag, d in 전체.items():
        if not d:
            continue
