...
#
# `cargo xtask check` 의 "스키마 정합" 은 정상 상태에서 `양방향 0건` 을 산출한다. 그런데
# **아무것도 세지 않는 검사도 0 을 돌려준다.** 그래서 스키마를 한 자리씩 망가뜨리고
...
# 검사가 실패해야 정상인 경우.
#
# ⚠ **변형이 파일을 실제로 바꿨는지 먼저 잰다** (2026-08-15 · F09 가 밟았다).
#
