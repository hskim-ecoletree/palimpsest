...
"""F10 대조 — 서술물 인입. **합격선은 재기 전에 등록됐다** (`corpus/criteria.toml` `[f10]`).
    ...
    ⑤ 동점 후보를 확정하지 않는다 — **일부러 동점을 만들어** 잰다
...
# 대조가 꺼지는 형태 — 이 스크립트가 막는 것 다섯
  ...
  · **열여섯·열일곱(변형이 대상을 안 건드림 · 종류를 안 봄)** — 문서 변형을 **종류별로**
    하고, **무엇이 바뀌어야 하는지를 먼저 적고** 그대로 나오는지 잰다. 바뀐 파일 수와
    바이트 차이가 0 이면 멈춘다
...
사용:
    ...
    ./scripts/f10-verify.py --self         # palimpsest 자기 인입도 함께 싣는다 (편의 표본)
"""
...
def 걸러낸다(got: dict) -> tuple[list[dict], int]:
    """도구 설정 아래를 뺀다. **뺀 수를 함께 싣는다 — 조용한 생략 금지.**"""
...
def 해소(tmp: Path, tag: str, src: Path, pin: str) -> dict:
    ...
    # ★ **강 신호 부분모집단** — 확인된 신호를 **산출할 수 있었던** 조각들.
    #
    ...
    # **3분류 셋이 전부 산출되는가** — 하한이 없으면 전부 미결박으로 산출하는 인입기가 통과한다.
...
def 표본(전체: dict[str, dict]) -> None:  # noqa: C901
    ...
    여기서는 **표본을 만들고 층화가 성립하는지**까지 잰다 — 판정 자체는 게이트다.
    """
    ...
    if len(판단드는것) * 2 < len(뽑은):
        skip("① 층화",
             ...
             f"**대조 불가로 잰다**(`[f10.2].sample_selection` 규칙 2)")
    else:
        ok("① 층화", 값)
...
# ═════════════════════════════════════════════════════════════════════════════
# ⑤ 동점 — **일부러 만든다.** 자연 발생을 기다리면 대조가 조용히 멎는다
# ═════════════════════════════════════════════════════════════════════════════
...
def 동점(tmp: Path) -> None:
    ...
    doc.write_text(
        ...
        #    맨 앞에 둔다. **이것이 없으면 「전부 후보로 잡는다」가 통과한다.**
        f"---\ngrounds: [\"{확정파일}#{확정이름}\"]\n---\n\n"
        f"# 확정\n\n`{확정이름}` 하나를 가리킨다.\n\n"
        # ① 인라인 스팬 동점 — 같은 이름이 여럿
        f"# 스팬 동점\n\n`{중복[0]}` 를 가리킨다.\n\n"
    ...
    for 앵커, 기대 in [("스팬-동점", "candidates"), ("경로-동점", "candidates"), ("확정", "bound")]:  # noqa: B007
        ...
        if 앵커 == "확정":
            ...
            # ★ 미결박으로 뭉개도 반증이다 — *"여럿이라 못 좁혔다"* 와 *"신호가 없다"* 는 다르다.
            걸림.append(f"{앵커}: unbound — **동점을 미결박으로 뭉갰다**")
...
def 결정성(tmp: Path) -> None:
    ...
    if 첫["fragments"] < MIN_FRAGMENTS:
        ...
        return
    if 첫["minted"] < 1:
        fail("⑥ 결정성", "첫 인입에서 만들어진 개체가 0 이다 — 아래 단언이 아무것도 안 잰다")
        return
...
def f09_재측정(tmp: Path, tag: str, src: Path, pin: str, ext: str) -> None:
    ...
    **결박 14 · 붙은 것 2** 가 나왔다 — 등록이 미리 적어 둔 그대로였다.
    ...
    print(f"      결박 {걸린수} · 커밋 {len(커밋들)} 훑음 · 붙은 것 {len(표본)}")
    for row in 표본:
    ...
    if 걸린수 < SAMPLES_PER_CORPUS:
        ...
        ok(f"⑨ {tag} 표본",
           f"결박 {걸린수} · 붙은 것 {len(표본)} — **거짓 양성 판정은 게이트에 결박마다 한 줄로**")
...
def main() -> int:
    ...
    ap.add_argument("--self", dest="self_", action="store_true",
                    help="palimpsest 자기 인입도 산출한다 — **편의 표본이고 판정 대상이 비어 있다**")
    a = ap.parse_args()
    ...
    try:
        ...
        if a.self_:
            ...
            # 추출기가 없어 palimpsest 의 2층 심볼이 한 자릿수다. 참고로만 산출한다.
