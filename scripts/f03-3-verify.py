...
(옛 F02 §6 · F02 지붕 §2). 빠뜨리지 않았음을 지는 것은 손 목록
...
⚠ **`--bless` 는 판정의 자리에서 쓰면 안 된다.** 그 스위치는 골든을 **다시 쓰고 rc=0**
으로 끝난다 — 회귀를 없애는 것이 아니라 회귀의 증거를 지운다. 회차 게이트가 골든 대조를
조건으로 걸 때는 `--bless` 없이 돌리고, **골든 두 파일이 회차 전후로 같은 바이트인지를
`git diff` 로 함께 대야** 한다(회차 `2026-09-07-rust-scope-references` 의 `C4`).
축복은 **산출이 실제로 바뀌기로 정해진 회차**에서만, 움직인 목록을 판정에 적고 한다.
"""
...
def main() -> int:
    ...
    # ── ② 자동 적용 경로가 없다 ★ ──────────────────────────────────────────
    # **없는 것을 시험한다.** 있으면 옛 F03 §5 가 기각한 자동 재결박이 서 버린다.
