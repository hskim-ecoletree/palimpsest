...
def 상한과_카탈로그(tmp: Path) -> None:
    ...
    # ⚠ **조건 없이 적으면 그 문장이 언제부터 거짓인지 아무도 모른다.**
    #    2026-08-18 재고 처분 전까지 `crates/pal-mcp` 는 **실재했고**, 이 줄은 그동안
    #    내내 거짓을 출력했다. 조건을 단다.
    if (ROOT / "crates/pal-mcp").exists():
        fail("⑦ MCP 경로", "`pal-mcp` 가 되살아났다 — ADR-0025 의 되돌리기 조건이 걸렸다")
    else:
        skip("⑦ MCP 경로", "`crates/pal-mcp` 는 **폐기됐다**(ADR-0025 · 2026-08-18) — "
                           "**「CLI 만 섰으므로 절반 통과」로 안 적는다**. 그리고 이것은 "
                           "미구축이 아니라 **요구 철회**다")
...
def 자기_적용(tmp: Path) -> None:
    box = tmp / "self"
    box.mkdir()
    # ★ **2026-08-18 재고 처분이 옛 `docs/plan/features/F12-plan-binding.md` 를 지웠다.**
    ...
    #
    # ⚠ **그 다음 판은 경로 리터럴 안에 「옛」을 밀어 넣어 검사 19 를 달랬다**
    #    (`ROOT / "옛 docs/…"` · 독립 리뷰 14 라운드가 잡았다). 그 경로는 **어떤
    #    상태에서도 존재하지 않으므로** 아래 갈림이 조건 없이 고정됐고, 같은 블록이
    #    적은 *"다시 겨눈다"* 는 **코드로 도달 불가한 약속**이 됐다.
    #    ★ **장치가 거짓말을 요구하면 그것은 대조가 아니다** — 「옛」은 **산문의**
    #    표기 규약이지 `Path` 인자의 규약이 아니다.
    #
    # ★ **대신 살아 있는 자리에서 후계를 찾는다.** 새 지형이 계획 문서를 세우면
    #   이 자리는 **스스로 다시 겨눠진다** — 표시가 아니라 장치다.
    후보 = sorted((ROOT / "docs/plan").glob("**/F12*.md"))
    if not 후보:
        skip("⑪ 자기 적용",
             "잴 계획 문서가 없다 — 옛 `docs/plan/features/F12-plan-binding.md` 가 "
             "**폐기됐다**(2026-08-18 재고 처분 · ADR-0025 로 지형이 바뀌었다). "
             "「아직 못 쟀다」가 아니라 **「잴 대상이 없어졌다」**다. "
             "`docs/plan/**/F12*.md` 가 다시 서면 이 자리가 스스로 다시 겨눈다 — "
