...
def 호스트_없는_코어(r: 결과) -> None:
    ...
    # `pal-mcp` 는 **폐기됐다**(ADR-0025 · 2026-08-18). 그러므로 *"어댑터를 빼도
    # 통과한다"* 는 **모집단 0** 이고 통과로 세지 않는다(ADR-0002).
    # ⚠ 사유가 바뀌었다 — 「아직 없다」가 아니라 **「영영 없다」**다.
    if (ROOT / "crates/pal-mcp").exists():
        r.fail("② 어댑터 부재 통과", "`pal-mcp` 가 되살아났다 — ADR-0025 의 되돌리기 조건")
    else:
        r.기권("② 어댑터 부재 통과", "`pal-mcp` 는 폐기됐다(ADR-0025) — 모집단 0")
