...
fn 요구하는_자리(
    ...
    let intent = format!("{회차_뿌리}/{회차}/intent.md");
    if matches!(처분, "축소" | "전환" | "범위밖") || 출처 == "사전부검" || 사전처분 != "해당없음" {
        return Ok(intent);
    }
    if 경로 == "(경로 없음)" {
        return Err("경로 없음");
...
fn check_finding_closure(root: &Path) -> Result<String> {
    ...
    for p in &산출 {
        ...
        for (i, line) in text.lines().enumerate() {
            ...
            let 상태 = v.get("상태").and_then(|x| x.as_str());
            ...
            let Some(sha) = v.get("닫은커밋").and_then(|x| x.as_str()) else { continue };
    ...
    Ok(format!(
        "잰 것 {잰_것}행 · 발화 {발화_수} · **감사 대기 발화 {대기_발화}** · \
         원리상 못 잼 {못_잼_글} · 이 이력에서 안 보임 {안_보임}행 · 열림 {열림}행 · SHA {}개",
        캐시.len()
    ))
}
...
mod 요구하는_자리_시험 {
    ...
    #[test]
    fn 기각은_원리상_못_잰다() {
        assert!(요구하는_자리("r", "독립리뷰", "기각", "해당없음", "a.rs").is_err());
    }

    /// 사전부검 발견의 `경로` 는 **예측된 파손 지점**이지 고침 지점이 아니다.
    #[test]
    fn 사전부검은_계획_문서를_요구한다() {
        let 자 = 요구하는_자리("r1", "사전부검", "정정", "해당없음", "xtask/src/main.rs").unwrap();
        assert!(자.ends_with("r1/intent.md"), "{자}");
    }

    /// 독립 리뷰의 정정은 **그 좌표**를 요구한다.
    #[test]
    fn 리뷰_정정은_좌표를_요구한다() {
        let 자 = 요구하는_자리("r1", "독립리뷰", "정정", "해당없음", "xtask/src/main.rs").unwrap();
        assert_eq!(자, "xtask/src/main.rs");
    }

    /// 좌표가 없거나 저장소 밖이면 **못 잰다** — 조용히 통과시키지 않는다.
    #[test]
    fn 좌표가_없거나_밖이면_못_잰다() {
        assert!(요구하는_자리("r", "독립리뷰", "정정", "해당없음", "(경로 없음)").is_err());
        assert!(요구하는_자리("r", "독립리뷰", "정정", "해당없음", "/tmp").is_err());
    }

    /// 범위밖·축소·전환은 계획 문서를 요구한다 — 개정과 승격이 거기 산다.
    #[test]
    fn 범위밖은_계획_문서를_요구한다() {
        for ch in ["축소", "전환", "범위밖"] {
            let 자 = 요구하는_자리("r2", "독립리뷰", ch, "해당없음", "a.rs").unwrap();
...
/// §10 이 금지한 **네 이름**. 종료 보고의 절머리가 이것과 **정확히 같으면** 실패다.
///
/// ★ 왜 넷인가 — 이 이름들은 **할 수 있었는데 안 한 것**(잔여)과 **원리상 못 하는
/// 것**(경계)을 한 칸에 섞는다. 섞이면 앞의 것이 뒤의 것 외양을 입는다.
/// 가르는 문장: **「이 회차에서 그것을 할 수 있었는가.」**
///
/// ⚠ **부분 문자열로 안 잡는다.** `## 다음 회차가 받는 것` 은 §10 이 **정한 이름**이지
/// 우회가 아니다. 그리고 규약 **밖**의 새 이름은 이 자가 원리상 못 잡는다 — 그것이
/// §11 의 음성 대조가 말한 구멍이고, 게이트가 그 사실을 적는다.
const 금지된_절_이름: &[&str] = &[
    "의도적으로 안 한 것",
    "확인 못 한 것",
    "추론(확인 아님)",
...
mod 종료_보고_시험 {
    ...
    /// ⚠ **규약이 정한 이름을 우회로 오인하지 않는다.**
    #[test]
    fn 규약이_정한_이름은_금지가_아니다() {
        for ok in ["다음 회차가 받는 것", "남지 않은 것", "범위 밖", "원리상 못 잰 것", "능력 부재"] {
            assert!(!금지된_절_이름.contains(&ok), "{ok}");
        }
        assert!(금지된_절_이름.contains(&"다음으로 넘기는 것"));
    }
}
...
fn check_ledger_pair(root: &Path) -> Result<String> {
    ...
    for 회차 in &회차들 {
        ...
        //
        // ★ 사전부검 R3 이 **더 넓은 문짝**을 찾았다: 「끝난 회차」의 유일한 기계
        //   정의가 `report.md` 존재인데, **그 파일이 있어야 한다고 요구하는 검사가
        //   하나도 없었다.** 실측 — `2026-08-18-round-protocol/state.md` 가
        //   *"단계: 종료"* 라 적었는데 그 디렉터리에 종료 보고도 게이트도 없고,
        //   판정문은 그것을 「게이트 없음」으로만 불렀다.
        //   **종료 보고를 안 쓰는 한 수가 C1·G1·G2 셋을 동시에 무력화한다.**
        let state = 뿌리.join(회차).join("state.md");
        if !끝났나 && state.is_file() {
            let s = std::fs::read_to_string(&state)?;
            // 「단계」 줄이 종료를 말하는가 — 산문이라 낱말로 본다.
            if s.lines().any(|l| {
                let l = l.trim();
                (l.starts_with("## 지금 단계") || l.contains("단계:") || l.contains("단계**"))
                    && l.contains("종료")
            }) || s.lines().any(|l| l.trim_start().starts_with("**종료**")) {
                if 보고없음_유예.iter().any(|x| x == 회차) {
                    보고없음_유예_발화 += 1;
                } else {
                problems.push(format!(
                    "{회차}: `state.md` 가 **종료**라고 적었는데 `report.md` 가 없다 — \
                     §10 이 종료 보고의 자리를 정했고, 그것이 「끝난 회차」의 유일한 \
                     기계 정의다. 안 쓰면 이 검사와 종료 보고 검사를 한 수로 비껴간다"
                ));
                }
            }
        }
