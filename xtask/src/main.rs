...
//    그리고 그 이름의 **경로 구분자가 Windows·Linux 에서 어떤 모양인지 아무도 안 쟀다**
//    (macOS 실측은 슬래시). 등록하는 날 그것부터 재라.
...
fn test(root: &Path) -> Result<()> {
    ...
    match 판정한다(모두_섰나, 보고가_다_있나, &실패, &등록) {
        판정::시험을_못_돌렸다 => bail!(
             ...
             **한 줄도 없다.**\n    \
             ⚠ 이것은 「시험이 실패했다」가 **아니다** — 위 stderr 의 `error[E…]` 를 \
             읽으십시오. 실패 집합은 비어 있고, 그 빈 집합은 **초록의 근거가 아니다.**"
        ),
        판정::어긋났다(problems) => {
            bail!("시험 결과가 등록과 다르다:\n    {}", problems.join("\n    "))
        }
...
///
/// 없으면 시험이 하나도 안 돈 것이다. `cargo` 는 그때도 rc≠0 을 내지만 stdout 에는
/// `test … ... FAILED` 가 **한 줄도 안 난다.**
///
/// **순수 함수다.**
