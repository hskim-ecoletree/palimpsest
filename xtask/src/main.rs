...
const WATCH_ENTRY_SITES: &[(&str, &str)] = &[
    ...
    //
    //   `pal radius` 는 **반경만 바꾼다.** 넓히면서 새로 드는 감시 원소의 `digest` 를
    ...
    ("crates/pal-cli/src/radius.rs", "반경만 바꿀 때 **base 커밋 투영에서** 읽어 만든다 — 스냅샷만 다르다"),
];
...
/// 문서는 전부 그대로 잰다. 그리고 **원인도 막았다** — `.claude/agents/` 의 산출물을
/// 산출하는 아홉이 *"저장소 안 문서는 링크가 아니라 코드 표기로 적어라"* 를 지고 있다.
///
/// ⚠⚠ **앞 판은 이 문장을 그 요구가 서기 전에 썼다**(독립 리뷰 R3 · 금지역). 그때
/// `.claude/agents/` 의 마지막 커밋은 착수 이전(`213d3ee`)이었고 `git grep '코드 표기'`
/// 가 0 건이었다 — **면제만 있고 원인 차단이 없는 상태를 「막았다」로 적은 것**이다.
/// [#134] 는 아직 열려 있다: 이 면제를 걷어낼 조건은 그 이슈가 진다.
///
...
fn 요구하는_자리(
    ...
    // ★★ **행이 스스로 처분 자리를 적었으면 그것이 이긴다.** (2026-09-10)
    //   아래 규칙 셋은 *"이 출처·이 처분은 대개 어디에 닿는가"* 의 어림이고, 이 칸은
    //   **그 행에 대한 사실**이다. 어림이 사실을 이기면 초록으로 만드는 남은 길이
    //   「닫은 것을 열린 채로 두는 것」이 된다 — 실측으로 열 행이 그 자리에 걸렸다.
    if !처분자리.is_empty() {
        return Ok(처분자리.to_owned());
    }
    let 의도 = format!(
        "{회차_뿌리}/{회차}/{}",
...
mod 요구하는_자리_시험 {
    ...
    /// **행이 스스로 적은 처분 자리가 규칙 셋을 다 이긴다.**
    ///
    /// `경로`(관측된 자리)와 처분이 닿은 자리가 갈리는 발견이 있고, `경로` 는 원 반환문이
    /// 정해서 사람이 못 고친다. 그때 이 칸이 없으면 초록으로 만드는 남은 길이
    /// **「닫은 것을 열린 채로 두는 것」**뿐이다.
    #[test]
    fn 처분자리가_적히면_그것이_이긴다() {
        // ① 사전부검 — 규칙 ①이 계획 문서를 요구하는 자리.
        let 자 = 요구하는_자리(&축(), "r1", "사전부검", "정정", "해당없음", "a.rs", "b.rs").unwrap();
        assert_eq!(자, "b.rs");
        // ② 사전처분이 붙은 자리 — 규칙 ②.
        let 자 = 요구하는_자리(&축(), "r1", "독립리뷰", "정정", "계획수정", "a.rs", "b.rs").unwrap();
        assert_eq!(자, "b.rs");
        // ③ 처분별 사전이 「의도」로 답하는 자리 — 규칙 ③.
        let 자 = 요구하는_자리(&축(), "r1", "독립리뷰", "축소", "해당없음", "a.rs", "b.rs").unwrap();
        assert_eq!(자, "b.rs");
        // ④ **기각은 안 이긴다** — 아무것도 안 고치는 처분은 무엇이 적혀 있든 못 잰다.
        assert!(요구하는_자리(&축(), "r", "독립리뷰", "기각", "해당없음", "a.rs", "b.rs").is_err());
        // ⑤ 비어 있으면 옛 규칙 그대로다.
        let 자 = 요구하는_자리(&축(), "r1", "사전부검", "정정", "해당없음", "a.rs", "").unwrap();
        assert!(자.ends_with("/intent.md"), "{자}");
    }
}
    ...
    for p in &산출 {
        ...
        for (i, line) in text.lines().enumerate() {
            ...
            let 자 = 요구하는_자리(
                ...
                v.get("처분자리").and_then(|x| x.as_str()).unwrap_or(""),
            );
            let 자 = match 자 {
                Ok(자) => 자,
                Err(why) => {
