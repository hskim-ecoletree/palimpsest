...
/// 등록이 가리킬 수 있는 **플랫폼 — 셋뿐이고, 여기 없는 것은 적을 수가 없다.**
///
...
/// - [`등록은_원리상_불가능한_것만_담는다`] 는 **날것의 `외침` 을 훑지만 이 필드를 안 본다**
///   — 비었나 · 까닭이 20 넘나 · 금지 낱말이 있나, 셋뿐이었다. 그래서 **초록**.
...
/// 그리고 그것이 **다른 방어 하나를 되살린다**: 모든 등록이 정확히 한 플랫폼에서 살아
/// 있으므로, 시험 **이름** 이 틀리면 그 플랫폼에서 [`등록과_댄다`] 가 양방향으로 운다
/// (「등록되지 않은 실패」 + 「등록됐는데 안 났다」). 앞 판은 플랫폼 오타 하나가 그
/// 방어까지 함께 껐다.
...
#[cfg(test)]
mod 외침_판정_tests {
    use super::{등록과_댄다, 등록된_외침, 시험이_돌았나, 실패한_시험들, 외침, 판정, 판정한다};
