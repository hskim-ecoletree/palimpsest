...
const 어색한_표현: &[(&str, &[&str], bool)] = &[
    ...
    ("~가 산다", &["가 산다", "가 사는", "만 산다", "도 산다", "개가 산다"], false),
    ("~ 선다", &["선다"], true),
    ("~를 낸다", &["낸다", "냈다", "내는", "낼 ", "낸 "], true),
    ("~ 센다", &["센다"], true),
    ("켜다", &["켜다", "켠다", "켜는", "켜진", "켜져", "켜고", "꺼진", "꺼져"], true),
    ("접다", &["접는다", "접었", "접는 문", "접지", "접고", "접어", "접은", "접힌", "접히", "접을"], true),
    ...
    for f in 파일들 {
        ...
        for (n, line) in text.lines().enumerate() {
            ...
            for (이름, _) in 어색한_표현_이_든_줄(line) {
                hits.push(format!("{}:{} 「{이름}」", 상대_경로(root, &f), n + 1));
            }
        }
    }
    if !hits.is_empty() {
