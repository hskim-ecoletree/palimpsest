...
def main(착수, 의도파일=None, 종료="HEAD"):
    ...
    #   (실측 2026-08-22). 그리고 걷기만 하면 ③④⑤⑥ 이 **거짓 0** 을 낸다.
    #   그래서 칸마다 가드를 달고 **0 이 아니라 「못 셌다」**를 낸다.
    범위빔 = 전체 is None
    if 범위빔:
...
def 해악칸(머리, 행):
    """⑨ 해악 게이트 — 규약 §11 ② 를 **기계가 낸다.**

    §11 ② 는 *"발견이 몇 개 남았나가 아니라 **남은 것이 무엇을 깨뜨리나**로 판정한다"*
    이고, 금지역·실패가 남아 있으면 **막힘**이다.

    ★ **「남은」을 재는 축이 열림 축이다** (레코드 스키마 3). 그것이 없으면 처분된
      발견까지 남은 것으로 세어져 **언제나 막힘**이 된다 — 이 회차의 레코드는
      금지역을 수십 건 담고 있고 전부 이미 닫힌 것이다. 사전부검 승격 2 가 그것을 잡았다.
      ⚠ 수를 여기 안 적는다 — `record.py count` 가 센다. 한 번 적었더니 즉시 낡았다.
    ...
    막힘 = 분포.get('금지역', 0) + 분포.get('실패', 0)
    if 막힘:
        print(f"⑨ 해악 게이트    **막힘** — 열린 금지역 {분포.get('금지역', 0)} · "
              f"열린 실패 {분포.get('실패', 0)}")
    else:
