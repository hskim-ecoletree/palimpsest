...
★ **⑦⑧⑨ 는 원천이 다르다.** ①~⑥ 은 git 이 답하고 ⑦⑧⑨ 는 **기록된 판정**이 답한다
...
# ★ **상자를 읽는 자는 `record.py` 하나다.** 여기서 정규식을 다시 쓰지 않는다 —
#   앞 회차가 `종류` 축을 두 벌로 두었다가 C2-b 반증을 맞았고, 답이 「불러서 쓴다」였다.
#   설치본도 같은 `bin/` 에 둘을 놓으므로 이 import 가 양쪽에서 성립한다.
...
def 라운드번호(h):
    ...
    # 연쇄 깊이가 **13 → 11** 로 틀린 수를 산출한다(실측 2026-08-19 · 독립 리뷰 1 라운드가 잡았다).
    #
    # ★ **이것이 회고가 C1-b 에서 실측한 병이 장치 안에서 재발한 것이다** —
    # *"축은 커밋 접두사에 걸지 않는다. 표기 규약이 회차 안에서 태어나면 갈린다."*
    # 그래서 **서사가 아니라 기계 표기 하나만** 읽고, 없으면 `None` 을 산출한다.
    # 없는 것을 추측으로 메우지 않는다.
...
def main(착수, 의도파일=None, 종료="HEAD"):
    ...
    # ★ **조기 return 을 안 한다.** 2026-08-19 가 ⑦⑧ 만 조기 return 밖으로 뺐고
    #   ②③④⑤⑥ 은 그대로 삼켜졌다 — 빈 범위에서 **그 다섯 칸이 아예 안 떴다**
    #   (실측 2026-08-22). 그리고 걷기만 하면 ③④⑤⑥ 이 **거짓 0** 을 산출한다.
    #   그래서 칸마다 가드를 달고 **0 이 아니라 「못 셌다」**를 산출한다.
    범위빔 = 전체 is None
    if 범위빔:
    ...
    # ② 미판정 잔액
    # ★ 정규식을 여기서 다시 쓰지 않는다 — `record.py` 의 `조건들` 이 유일한 자리다.
    ...
    if 의도파일 and os.path.exists(의도파일):
        ...
        if not cs:
            print("② 미판정 잔액    — **못 셌다** (`## 완수 조건` 절에 조건이 없다. 0 이 아니다)")
        elif 오류:
            print(f"② 미판정 잔액    {열림} / {len(cs)}"
                  f"   ⚠ **형식 이전** — 형식 오류 {오류} 건. 이 회차는 대조 밖이다")
        else:
            print(f"② 미판정 잔액    {열림} / {len(cs)}")
    else:
    ...
    # ③ 진자 P1
    if 범위빔:
        # 한글 폭 때문에 `{:<16}` 이 안 맞는다 — 다른 칸들과 같이 리터럴로 맞춘다.
        for 칸 in ("③ 진자 (P1)     ", "④ 연쇄 깊이      ",
                   "⑤ 라운드         ", "⑥ 승격 횟수      "):
            print(f"{칸}— **못 셌다** (커밋 범위가 비었다. 0 이 아니다)")
    진자 = [] if 범위빔 else [p for p, k in 만짐.items() if k >= 2 and not sh(f"git diff {착수} {종료} -- '{p}'").strip()]
    if not 범위빔:
    ...
    # ④ 연쇄 깊이
    # 라운드 표기가 붙은 커밋이 있으면 라운드로, 하나도 없으면 **커밋 수로** 산출한다.
    ...
    #   앞 판이 *"라운드가 붙은 커밋 N"* 이라 적어 거짓 신호를 산출했다(독립 리뷰 2 라운드).
    표기달린커밋 = sum(1 for h in 커밋 if 라운드번호(h) is not None)
    if not 범위빔: print(f"⑤ 라운드         {최신라운드 if 최신라운드 else '—'}"
    ...
    승격 = 0 if 범위빔 else len([h for h in 커밋 if '[승격]' in sh(f"git show -s --format=%s {h}")])
    표기있음 = (not 범위빔) and any('[승격]' in sh(f"git show -s --format=%s {h}") or '[R' in sh(f"git show -s --format=%s {h}") for h in 커밋)
    if not 범위빔: print(f"⑥ 승격 횟수      {승격}"
    ...
    print()
    if 범위빔:
        print("⚠ 범위가 비었다 — 인자: python3 dashboard.py <착수커밋> [의도파일] [종료커밋]")
        print("  ② 와 ⑦⑧⑨ 는 커밋 범위를 안 쓰므로 위에 그대로 났다.")
    print("⚠ 이 계기판은 **지난 라운드들의 잔액**을 산출한다.")
    print("  지금 착수할 수정이 진자를 만드는지는 원리상 못 말한다.")
    print("⚠ ①~⑥ 은 `<착수>..<종료>` **커밋 범위**를 재고, ⑦⑧⑨ 는 **레코드 파일 전체**를 잰다.")
...
# ⚠⚠ **조기 return 마다 ⑨ 도 산출한다.** (독립 리뷰 R7)
#   `A1`·`A2` 가 등록한 병 — *"칸이 조기 return 에 삼켜진다"* — 이 **이 회차가 새로
#   만든 ⑨ 칸에 다시 났다.** 레코드가 없는 회차에 대면 아홉 칸이 아니라 여덟이 떴고,
#   같은 출력의 꼬리말은 *"⑦⑧⑨ 는 레코드 파일 전체를 잰다"* 라 적고 있었다.
def 해악칸(머리, 행):
    """⑨ 해악 게이트 — 규약 §11 ② 를 **기계가 산출한다.**

    §11 ② 는 *"발견이 몇 개 남았나가 아니라 **남은 것이 무엇을 깨뜨리나**로 판정한다"*
    이고, 금지역·실패가 남아 있으면 **교착**이다.

    ★ **「남은」을 재는 축이 열림 축이다** (레코드 스키마 3). 그것이 없으면 처분된
      발견까지 남은 것으로 세어져 **언제나 교착**이 된다 — 이 회차의 레코드는
      금지역을 수십 건 담고 있고 전부 이미 닫힌 것이다. 사전부검 승격 2 가 그것을 잡았다.
      ⚠ 수를 여기 안 적는다 — `record.py count` 가 잰다. 한 번 적었더니 즉시 낡았다.

    ⚠ **스키마 2 회차는 0 이 아니라 「형식 이전」이다.** 소급 이주를 안 하기로 했으므로
      옛 파일에는 이 축이 원리상 없다. 0 으로 내면 **「닫을 수 있다」라는 거짓 판정**이
      된다 — 안 잰 것을 통과로 적는 자리다.
    """
    버전 = (머리 or {}).get('schema_version')
    if not isinstance(버전, int) or 버전 < 3:
        print(f"⑨ 해악 게이트    — **형식 이전** (레코드 스키마 {버전} · 열림 축이 없다. 0 이 아니다)")
        return
    열림 = [o for o in 행 if o.get('상태') == '열림']
    축_없음 = [o for o in 행 if o.get('상태') is None]
    if 축_없음:
        print(f"⑨ 해악 게이트    — **못 셌다** ({len(축_없음)} 행에 `상태` 가 없다)")
        return
    분포 = collections.Counter(o.get('해악도') for o in 열림)
    교착 = 분포.get('금지역', 0) + 분포.get('실패', 0)
    if 교착:
        print(f"⑨ 해악 게이트    **교착** — 열린 금지역 {분포.get('금지역', 0)} · "
              f"열린 실패 {분포.get('실패', 0)}")
    else:
        print(f"⑨ 해악 게이트    **닫을 수 있다** — 열린 발견 {len(열림)} "
              f"(금지역 0 · 실패 0)")
    if 열림:
        print("                  " + " · ".join(f"{k} {v}" for k, v in 분포.most_common()))
    print(f"                  닫힘 {len(행) - len(열림)} / {len(행)} — "
          "시점은 각 행의 `닫은커밋` 이 진다")
