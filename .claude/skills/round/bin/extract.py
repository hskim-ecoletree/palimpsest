...
def _라벨들(불릿, 별칭):
    """이 형식이 아는 축 라벨 전부 — 한 줄 메타를 가르는 자리와 읽는 자리가 함께 쓴다."""
    out = set(불릿.values())
    for 이름들 in 별칭.values():
        out.update(이름들)
    return sorted(out, key=len, reverse=True)


def _한줄메타를_가른다(line, 불릿, 별칭):
    """`- 모집단: A · 유효성: B · 해악도: C` 를 불릿 셋으로 가른다.

    **알려진 라벨 앞의 ` · ` 에서만 자른다.** 요약과 좌표에도 그 구분자가 쓰이므로
    무조건 자르면 값이 잘린다.
    """
    ...
    본문 = m.group(1)
    라벨 = _라벨들(불릿, 별칭)
    패턴 = r"\s+·\s+(?=(?:\*\*)?(?:" + "|".join(re.escape(l) for l in 라벨) + r")\s*:)"
    return ["- " + 조각 for 조각 in re.split(패턴, 본문) if 조각.strip()]


def _불릿을_읽는다(현재, line, 불릿, 별칭, 프로필, 역사별칭):
    ...
    for 칸, 라벨들 in 후보들.items():
        ...
        if any(이름 == 라벨 or 이름.startswith(라벨) for 라벨 in 라벨들):
            # 같은 항에 옛 보조 축(`획득`)과 정본 축(`유효성`)이 함께 있으면 정본
            # 축이 이긴다. 보조 축을 먼저 본 순서 때문에 뒤의 명시 값을 삼키면
            # 현재 형식의 산출까지 달라진다.
...
def 사전부검(text, 별칭, 불릿, 없음, 프로필="current", 역사별칭=None):
    ...
    for line in text.split("\n"):
        ...
        if line.startswith("### "):
            ...
            continue
        if 현재 is not None:
            # ★★ **한 줄에 이어 적은 메타를 가른다** (2026-09-08 · #132).
            #   에이전트가 `- 모집단: 저장소 · 유효성: 거짓 · 해악도: 미관 · 어디가
            #   걸리나: …` 처럼 축 넷을 한 불릿에 적는다. 안 가르면 첫 축만 읽히고
            #   나머지가 **빈 값**으로 나와 원장과 갈린다(실측: 조건 평가 두 라운드에서
            #   여섯 항 × 두 칸).
            #   ⚠ **알려진 라벨 앞에서만 가른다** — 요약·좌표에도 ` · ` 가 쓰인다.
...
# ── enum 축으로 좁힌다 — **사람이 손으로 하던 일을 기계가 한다** (2026-09-08 · #132)
#
# 에이전트는 축 하나에 값을 **둘 적는다**(`모집단: 자기장치 · 저장소`) 거나 값 뒤에
# 근거를 붙인다(`유효성: 추정(거친 계수 · 방법 아래 명시)`). 원장의 enum 은 값 하나만
# 받으므로 앞 세션이 그것을 **손으로** 줄였고, 그러자 「기계 칸이 추출기 산출과 갈린다」가
# 서른아홉 건으로 빨개졌다 — *"손으로 옮기지 않는다"* 와 *"enum 안이어야 한다"* 가
# 서로를 배제한 자리다(#132).
#
# **그 축소를 여기로 옮긴다.** 규칙은 하나다 — **원문에 나타난 순서대로 첫 enum 값**.
# 그러면 두 원장이 같은 답을 내고, 사람이 판단한 것이 아니라 스크립트가 정한 것이 된다.
#
# ⚠ **버린 값을 지우지 않는다.** 원문은 `r<n>-raw.md` 에 그대로 있고, 이 함수는
#   원장 칸만 좁힌다. 무엇이 버려졌는지 알려면 원문을 보면 된다.
_ENUM = {
    "모집단": ("원의도", "저장소", "자기장치", "회차기록", "규약"),
    "유효성": ("참", "추정", "거짓"),
    "해악도": ("금지역", "실패", "거짓신호", "미관"),
...
def enum으로_좁힌다(항들):
    """`모집단`·`유효성`·`해악도` 를 enum 값 하나로 줄인다. 못 찾으면 원문 그대로 둔다."""
    for d in 항들:
        for 칸, 값들 in _ENUM.items():
            원 = 정규화(str(d.get(칸, "")))
            if not 원 or 원 in 값들:
                continue
            자리 = [(원.find(v), v) for v in 값들 if v in 원]
            if 자리:
                d[칸] = min(자리)[1]
    return 항들
