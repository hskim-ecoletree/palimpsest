...
| **그래프 엔지니어링** | 2026-07-18 | Peter Steinberger 트윗에서 발화 | 실행을 **여러 노드로 분리해** 프로그래머블하게 — 다중 에이전트 조직 |

**루프 → 그래프의 방향을 세운 논리를 먼저 적어 둔다** (이 조사가 대조한 진단의 전제이며, 외부 문헌의 주장이 아니다).

> 루프 엔지니어링은 사람을 시스템으로 교체하지만, 그 시스템이 돌리는 **절차는 선형 반복일 수밖에 없다.** 계획→실행→검증→수정이 한 줄로 이어지고, 다음 회차는 앞 회차가 끝나야 시작된다. 그러나 실제 작업은 **여러 갈래로 갈라지거나 여러 개의 독립 루프로 동시에 진행될 수 있다.** 실행을 노드로 쪼개고 그 사이의 의존만 선언하면 선형 제약이 풀린다 — **그래프는 루프의 선형성을 푼 형태**라는 관점에서 루프 > 그래프의 순서가 나온다.

이 관점에서 보면 각 층은 **직전 층의 제약을 하나씩 푸는** 계열이다. 컨텍스트는 단발 질의의 기억 없음을, 하네스는 컨텍스트만으로 강제되지 않는 규율을, 루프는 사람이 매번 프롬프트해야 하는 병목을, 그래프는 루프의 선형성을 푼다. **다만 아래 (1)이 지적하듯 제약을 푸는 것과 결함을 고치는 것은 다르다** — 각 층은 아래층의 결함을 함께 증폭한다.
...
- **그래프-A (실행 위상)**: 실행 자체를 노드로 분리해 분기·동시 진행을 가능하게 하는 것. 여러 에이전트·평가·정책을 노드로 잇는다. Steinberger 계열 담론과 **위에 적은 루프→그래프 논리가 가리키는 것이 이쪽이다.**
...
**게이트가 merge에 남은 것과 그 자리가 막히는 것은 다른 사실이고, 후자의 원인은 사람과 에이전트의 처리량 차이다.** [구조적 논증 — 아래 인용 수치는 등급 B]

에이전트가 구현의 대부분을 맡으면서 **사람이 수 일에서 수 개월 걸릴 일이 몇 분 만에 끝난다.** 그러나 **일의 본질도, 해야 할 일의 총량도 줄어들지 않았다.** 줄어든 것은 그 일을 **생산하는 데 걸리는 시간**뿐이다. 검토해야 할 결정의 수, 확인해야 할 경로의 수, 따져야 할 상호작용의 수는 그대로다.

귀결은 **부하의 압축**이다. 예전에는 수 개월에 걸쳐 나뉘어 도착하던 분량 — 그 사이사이에 사람이 이해를 쌓을 시간이 있던 분량 — 이 **한 번에 도착한다.** 리뷰는 생산과 같은 배율로 빨라지지 않으므로, 게이트는 자리를 지킨 채 대기열만 길어진다. 이것이 §4.1이 인용하는 LinearB 수치의 형태다 — **머지 PR 98%↑, 그런데 PR 리뷰 시간 91%↑, AI 생성 PR은 리뷰어가 집어들기까지 4.6배 대기.** 위 Anthropic 사례의 "1인당 일일 머지량 8배"도 같은 압축을 반대편에서 보여준다.

**그리고 더 나쁜 것은 양이 아니라 컨텍스트의 부재다.** 사람이 수 개월에 걸쳐 하던 일에는 중간 상태가 남는다 — 무엇을 먼저 시도했고 왜 버렸는지, 어느 지점에서 방향을 틀었는지, 어떤 제약 때문에 최종 형태가 이렇게 됐는지. 그 이력이 커밋·리뷰·대화에 흩어져 있어도 **리뷰어가 그 위에서 판단할 수 있었다.** 에이전트가 같은 일을 몇 분에 끝내면 **사람이 받는 것은 최종 diff 하나뿐이고, 그 형태에 이르기까지의 결정은 에이전트의 컨텍스트 안에서만 존재했다가 세션과 함께 사라진다.**

즉 사람은 **더 많은 양을, 더 짧은 시간에, 더 적은 근거로** 판정해야 한다. 세 요인이 같은 방향으로 겹친다. 이것이 §4.1의 "거의 맞는데 완전히는 아닌" 결과물이 특히 위험한 이유이기도 하다 — 판정에 쓸 시간과 근거가 모두 줄어든 상태에서, 실패가 요란하지도 않다.

이 절은 이후 세 지점의 전제가 된다. **§5.3의 컨텍스트 리셋 + 구조화된 인계**는 에이전트 쪽에서도 같은 소실이 일어나기 때문에 필요한 장치이고, **§7.1의 저자성 붕괴**는 그 소실이 조직의 지식 지표로 번진 형태이며, **`boxwood` 관측 §7**([2026-08-01-boxwood-fix-taxonomy.md](2026-08-01-boxwood-fix-taxonomy.md#7-발견-5--수정이-결함의-클래스가-아니라-발현-하나를-닫는다))은 그 실물이다 — 확립된 규칙이 그 커밋의 diff 안에만 존재했고, 한 달 뒤 같은 좌표의 작업자가 규칙을 처음부터 다시 발견해야 했다.

**반증 조건**: 리뷰 처리량이 생성 처리량과 같은 배율로 오르면(예: 검증을 에이전트로 위임하는 것이 실제로 작동하면) 이 병목은 해소된다. 그 위임이 어디까지 듣는지가 §5.4~5.6의 주제이며, 현재 측정으로는 **검증자 자신이 하한을 모른다.**

### 병목의 귀결 — 세 개의 답할 수 없는 질문

병목이 된 것은 배제로 이어진다. **다만 정확히 적어야 한다 — 사람이 게이트에서 치워진 것이 아니다.** 위에서 측정한 대로 경계는 여전히 merge에 있고 승인은 사람이 한다. 배제된 것은 **자리가 아니라 판정력**이다. 승인은 남았는데 **이해하고 승인하는 것이 아니게 됐다.**

이 형태는 문헌에 이미 있다. §4.1의 **AI 생성 PR이 리뷰어에게 집어들리기까지 4.6배 대기**, §6.3이 적는 **"표시해도 사람은 확인을 그만둔다"**, §4.1의 **"AI 생성 코드 디버깅이 더 오래 걸린다(45%)"** — 전부 사람이 게이트에 서 있으면서 판정을 유예하거나 포기하는 형태다. [등급 B]

**그 결과가 지금의 상태다.** 코드베이스는 커졌고 변화는 많았다. 그런데 다음 세 질문에 답할 수단이 없다.

| 질문 | 왜 답할 수 없나 | 실물 |
|---|---|---|
| **이것이 정말 의도에 부합하는 산출물인가** | 기능적 정확성을 다루는 층이 **미개발**이고(§4.3), 그 판정을 대신하던 게이트는 통과할 쪽이 쓴다(§4.2). 그리고 애초에 무엇을 의도했는지가 **에이전트의 컨텍스트에만 있었다가 사라졌다**(위 절) | Anthropic 1차 관측 — "코드 변경은 하지만 그 기능이 E2E로 동작하지 않는다는 것을 인식하지 못한다" |
| **내부에 어떤 잠재 위협·결함이 있는가** | 초록색 CI가 그것에 대해 아무 말도 하지 않는다. 검증은 **쓰기 쉬운 쪽에서 충족**되고 비싼 쪽은 비어 있다 | `boxwood` 관측: 절차 하네스가 적용되고 CI가 초록색인 코드베이스에서 3개월 **104건**. 리소스 가드 없는 엔드포인트 **271개 중 어느 것이 정당한 공백인지 판정할 기준이 없다** ([§4](2026-08-01-boxwood-fix-taxonomy.md#4-발견-2--가드-누락은-실재하고-잔여를-판정할-수단이-없다)) · 테스트 36개 도메인의 결함 수정은 **0건**, 결함은 테스트 0개 도메인에 몰려 있다 ([§6](2026-08-01-boxwood-fix-taxonomy.md#6-발견-4--테스트가-있는-곳과-결함이-나는-곳이-겹치지-않는다)) |
| **그 위에 새것을 얹어도 될 만큼 견고한가** | 쌓인 것이 **이해된 적이 없다.** 그리고 "누가 아는가"를 재던 수단도 함께 무효화됐다(§7.1) | GitClear — 생성은 늘고 **재구조화는 줄었다**(리팩터 라인 이동 70%↓, 교차 파일 재사용 35%↓) · `boxwood` — 같은 함수의 같은 결함 클래스가 **다섯 번 재발**했고, 규칙은 그 커밋의 diff 안에만 있었다 ([§7](2026-08-01-boxwood-fix-taxonomy.md#7-발견-5--수정이-결함의-클래스가-아니라-발현-하나를-닫는다)) |
...
**이것이 인지부채(cognitive debt)다.** 이 용어는 보통 개인의 인지 능력 저하로 소개되지만(§6.3에 그 정량이 있다), **이 절이 도달한 지점이 그 개념의 정확한 자리다** — 이해를 건너뛰고 얻은 속도가 지금 청구되지 않고 **나중에 이자와 함께 청구되는** 상태.

부채 은유가 정확한 이유는 세 가지다.

1. **원금은 건너뛴 이해다.** 몇 분 만에 끝난 일의 중간 결정을 아무도 따라가지 않았다. 그 순간에는 비용이 0이다.
2. **이자는 재발이다.** 나중에 같은 자리를 만지는 사람이 그 이해를 처음부터 다시 사야 한다. `boxwood` 관측 [§7](2026-08-01-boxwood-fix-taxonomy.md#7-발견-5--수정이-결함의-클래스가-아니라-발현-하나를-닫는다)이 실물이다 — 6월에 확립된 규칙이 그 커밋의 diff 안에만 존재해서, 한 달 뒤 같은 좌표의 작업자가 규칙을 다시 발견해야 했고 **그 사이 9개 필드가 유실된 채로 운영됐다.** 같은 함수의 같은 결함 클래스가 다섯 번 재발한 것이 이자 지급 내역이다.
3. **그리고 결정적으로 — 잔액을 볼 수 없다.**

세 번째가 기술부채와의 차이이고, 이 조사에서 가장 중요한 구분이다. **기술부채는 코드에 있으므로 관측 가능하다** — GitClear가 중복 81%↑, 재구조화 70%↓를 실제로 센다(§4.1). **인지부채는 사람의 머리와 사라진 에이전트 컨텍스트에 있으므로 잔액이 어디에도 표시되지 않는다.** 조사 범위에서 이것이 측정된 유일한 장소는 EEG 실험실이었다(§6.3). 코드베이스에는 계기판이 없다.

그래서 위 세 질문 중 **셋째("얹어도 될 만큼 견고한가")가 특히 답할 수 없다.** 앞의 두 질문은 적어도 무엇을 찾아야 하는지가 정해져 있지만, 셋째는 **잔액을 모르는 상태에서 추가 차입이 안전한지를 묻는 것**이기 때문이다.

그리고 §7.1이 보이듯 **부채를 갚을 사람을 찾는 수단(저자성)도 함께 무효화됐다.** `git blame`은 여전히 이름을 반환하지만 그 이름은 규칙을 아는 사람을 가리키지 않는다. 부채는 쌓이고, 잔액은 안 보이고, 채무자는 특정되지 않는다.

**이것이 §8이 정리하는 네 가지 대응과 남은 구멍의 배경이며, §9에서 백서와 만나는 지점이다.** 남은 구멍 중 셋 — **행동 하네스**(첫 질문), **낡음의 감지**(둘째), **의도의 별도 기록**(셋째) — 이 위 표의 세 행과 정확히 대응한다.

**용어 사용에 대한 주의**: "인지부채"를 이 절의 뜻(조직 수준의 이해 소실)으로 쓰는 것은 **이 보고서의 확장이다.** 원 연구(MIT, §6.3)가 측정한 것은 **개인의 에세이 작성 과제**이고 코딩으로의 전이조차 검증되지 않았다. 여기서 정당한 것은 **은유의 구조**(원금·이자·잔액 불가시성)를 빌려 쓰는 것이지, 원 연구의 수치를 이 상태에 적용하는 것이 아니다.

**이 절의 지위** [구조적 논증 — 인용 수치는 등급 A/B]: 위 인과("병목 → 판정력 상실 → 세 질문에 답할 수 없음")는 **이 조사가 관측들을 잇는 논증이지 어느 문헌이 직접 주장한 것이 아니다.** 각 칸의 근거는 결박되어 있으나 그 사이의 화살표는 결박되어 있지 않다. **반증 조건**: 리뷰어가 AI 생성 변경을 실제로 어느 깊이까지 판정하는지 측정한 연구가 나오면 두 번째 마디("배제된 것은 판정력")가 직접 검사된다. 조사 범위에서 그런 측정은 확인되지 않았고, 위 근거는 전부 **대리 지표**(대기 시간·디버깅 시간·자기보고 불신)다.
...
**세 방향 모두 같은 동력을 갖는다** — 위 절의 처리량 비대칭이다. 사람이 코드 앞에 서면 압축된 부하를 컨텍스트 없이 맞게 되므로, 산업은 사람을 **부하가 아직 압축되지 않은 상류(스펙)로 올리거나**, **압축된 부하를 감당할 수 있는 다른 에이전트로 옆으로 넘기거나**, **애초에 사람의 판정을 요구하지 않는 결정론적 하부구조로 내리는** 세 방향을 동시에 시도하고 있다.
...
### 9.0 이 조사가 가리키는 자리 — 이해를 사람에게 되돌리는 층

**논지 전체를 한 줄로 잇는다.** §3이 도달한 상태는 인지부채이고, 그 부채의 성질은 **잔액이 안 보인다**는 것이며, 남은 구멍 여섯 개(§8)는 전부 그 불가시성의 다른 이름이다. 그러므로 이 조사가 가리키는 자리는 **생성을 더 빠르게 하는 층이 아니라 이해를 사람에게 되돌리는 층**이다.

**"되돌린다"의 뜻을 정확히 해야 한다. 사람이 전부 읽는다는 뜻이 아니다.** 그것은 §3이 측정한 병목으로 곧장 되돌아가는 길이다 — 생산이 몇 분이고 열람이 몇 개월이면, 열람을 요구하는 해법은 정의상 작동하지 않는다. 백서 §4가 이 지점을 이미 정확히 적었다:

> "기계가 양쪽이 함께 가리킬 수 있는 기준을 들고 있을 때에만 **판정이 열람량에 비례하지 않게 된다.**"

**이해의 회수는 열람량을 늘리는 것이 아니라 판정을 열람에서 분리하는 것이다.** 그 형태로 §3의 세 질문에 각각 대응하면 이렇게 된다.
...
**세 줄 모두 "사람이 더 많이 읽는다"가 아니라 "사람이 읽지 않고도 판정할 수 있다"는 형태다.** 이것이 §3의 처리량 비대칭과 양립하는 유일한 방향이다.

**이 절의 지위** — 이것은 **조사가 가리키는 방향이지 백서에 대한 요구가 아니다.** 위 표의 오른쪽 열은 백서에 이미 있는 조항이고, 이 조사가 하는 일은 그 조항들이 겨냥한 실패가 실재함을 외부 증거로 확인하는 것뿐이다. **어느 조항이 이 방향을 실제로 달성하는가는 이 문서가 판정할 수 없다** — 제5부에 해당하는 [적대적 감사](2026-08-01-whitepaper-adversarial-audit.md)가 바로 그 조항들이 자기 목표를 달성하지 못할 수 있는 32가지 경로를 열거하고 있고, 그중 어느 것도 아직 판정되지 않았다.
