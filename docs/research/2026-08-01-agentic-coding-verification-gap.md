...
세 번째가 기술부채와의 차이이고, 이 조사에서 가장 중요한 구분이다. **기술부채는 코드에 있으므로 관측 가능하다** — GitClear가 중복 81%↑, 재구조화 70%↓를 실제로 잰다(§4.1). **인지부채는 사람의 머리와 사라진 에이전트 컨텍스트에 있으므로 잔액이 어디에도 표시되지 않는다.** 조사 범위에서 이것이 측정된 유일한 장소는 EEG 실험실이었다(§6.3). 코드베이스에는 계기판이 없다.
