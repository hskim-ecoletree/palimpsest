...
- 4는 schema 3의 `verify --all`이 이미 met인 command까지 다시 실행한 뒤에만 checkpoint를
  append하는 것으로 구현한다.
- 5는 command와 명시적 thesis·antithesis·synthesis judgment를 같은 aggregate digest에
  넣는 것으로 구현한다.
- 6은 current schema 3 `findings.jsonl`과 열린 축을 요구하고 열린 `금지역`·`실패`를
