...
| `pal uninstall` | ⚠ **이 줄은 재 본 적 없는 서술이었다 — 실측하면 걷힌다.** 전면 CRLF 변환(24 파일) 뒤에도 `doctor` 검사 1·2·5·7 이 ok 이고 `uninstall` 은 rc=0 으로 블록까지 걷어낸다. 격리 HOME 에서 두 판으로 쟀다 — HEAD 바이너리와 **착수 `v0.1.1` 바이너리 둘 다** 같다(2026-09-16 · 독립 리뷰 R1 발견 #6). 앞 판의 *"통째로 거부한다 … 걷어낼 방법이 없다"* 는 **거짓이고, 어느 회차가 낡게 만든 것도 아니다** |
