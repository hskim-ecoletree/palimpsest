...
| 파싱 | **`tree-sitter`** + 문법 넷 — `kotlin` · `java` · `javascript` · `typescript` | 빌드·툴체인 없이 blob 하나로 파싱된다. 오류 회복이 있어 `partial` 상태를 표현할 수 있다. **넷이 전부 1급이다**([지시 2026-08-12 §1](../instructions/2026-08-12-owner-direction.md)) |
