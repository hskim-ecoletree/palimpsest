...
- `ERROR`/`MISSING` 노드의 `span`을 `Site`로 기록하고 파일 상태를 `Partial`로. **`FileGraph.recovery_sites`가 그것이고 둘을 종류로 가른다** — `MISSING`은 너비가 0이라 span만으로는 "아무 데도 아닌 곳"과 같은 값이 된다.
- **회복 지점이 있는 파일의 심볼은 버리지 않는다.** 파싱된 부분에서 나온 심볼은 유효하고, 그 파일에 공백이 있다는 사실이 대장에 남는다.
- `ERROR` 노드 비율이 임계(자리표시 30% · `pal-core::budget`)를 넘으면 `Partial` 대신 `Unsupported`로 낮춘다 — 문법이 그 언어 버전을 못 따라가는 경우. **이유를 값으로 싣는다**(`UnsupportedReason::GrammarDefeated`): *"추출기가 없다"*와 뭉개면 대장이 로드맵의 자리를 가리키고 사용자가 거기서 고칠 곳을 찾는다.
  - **비율은 삼킨 넓이지 자리의 수가 아니다.** 자리 열 개보다 넓은 자리 하나가 더 나쁘다.
  - 강등된 파일의 **심볼은 싣지 않는다.** 못 읽었다고 적는 것과 그 파일의 선언을 대장에 싣는 것은 같이 설 수 없다.
...
| **`ERROR` 노드가 컨테이너를 삼킴** | 오류 회복이 클래스 전체를 ERROR로 묶으면 그 안의 메서드가 사라짐 | ~~ERROR 노드 **안쪽도 순회**해서 인식 가능한 선언을 건진다~~ → **정정: 세우지 않는다.** 아래 참조 | 회복 지점이 대장에 남으므로 조용하지 않다 |
| **Kotlin** | [R-03](../00-risks.md#r-03) — PSI 대비 구조 일치율 61.2% | 착수 전 측정(P0-preflight T7), 등급을 낮춰 선언 | 등급 L1로 고정하고 그 사실을 대장에 노출 |

### 정정 (2026-08-13 · #47) — **"ERROR 안쪽도 순회한다"를 세우지 않는다**

위 표의 셋째 줄은 *"오류 회복이 클래스 전체를 ERROR로 묶으면 그 안의 메서드가 사라진다"*는 **가정된 실패 모드**에 대한 대응이었다. 두 코퍼스 전수로 재 본 결과 **그 실패 모드가 일어나지 않는다**:
...
관측된 것은 **양 극단뿐이고 가운데가 없다**:

- `ERROR` 가 **토큰 몇 개**만 삼킨다 → 컨테이너도 그 안의 선언도 멀쩡히 나온다. 건질 것이 애초에 안쪽에 없다
- `ERROR` 가 **파일을 통째로** 삼킨다 → 안쪽에 파싱된 것이 하나도 없다. 순회해도 건질 것이 없다

후자가 이 코퍼스에서 27 건이고 그것의 처분은 순회가 아니라 **§3.4 의 임계 강등**이다(`Unsupported{GrammarDefeated}`). 그러므로 안쪽 순회는 **없는 문제에 대한 대응**이었다.

**세우지 않고 이 정정을 적는다** — [F01이 언어 인식 ④(내용 휴리스틱)에 내린 것과 같은 판단](../../gates/F01-repo-ledger.md)이고, 그때도 *"세우는 것이 합격선이 아니라 세울지 판단하고 근거를 적는 것이 합격선"*이었다. 판정 기록은 [`docs/gates/F02-2-partial.md`](../../gates/F02-2-partial.md).

**추출기의 순회 자체는 `ERROR` 자식으로 내려간다** — TypeScript 순회의 기본 가지가 그렇다. 세우지 않은 것은 *"안쪽에서 건진 심볼을 따로 표시하는 것"*이고, 표시할 심볼이 실물에 0 건이다.
