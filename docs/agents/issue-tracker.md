...
| | 어디 |
|---|---|
| 각 기능을 **어떻게** 만드는가 | 순서는 [순서표](../plan/02-order.md)가 지고 각 항목의 합격선은 그 항목을 집는 회차가 쓴다 ⟨독립 리뷰 R5 가 「비었다」를 정정했다⟩ — 옛 문서는 비었다 — 옛 `docs/plan/features/` 는 2026-08-18 에 지웠다([처분표](../plan/disposal-map.md)). 새 지형은 [순서표](../plan/02-order.md)가 졌다(2026-09-06) |
...
⚠ **이 규칙은 2026-08-18 에 무너졌다.** 옛 판은 *"이슈 본문은 기능 문서를 복제하지
않고 가리킨다"* 였는데, 그 문서 25 개를 지우자 **에픽 이슈 12 건이 빈 포인터**가 됐다
([처분표](../plan/disposal-map.md)).

★ **가리키기만 하는 것은 가리킬 것이 사라지는 순간 아무것도 아니다.** 그래서 이슈는
**자기 안에 판정 가능한 것**을 담는다 — 실제로 이 회차가 남긴 결함 이슈 아홉은 측정치를
본문에 품고 있어서 살아남았고, 가리키기만 하던 에픽 열둘은 닫혔다.

**`/implement`는 티켓을 닫지 않는다** (알려진 동작 — 커밋에서 끝난다). 이 저장소에서는 그것이 오히려 맞다: 게이트 판정은 사람이 `docs/gates/`에 기록하고, 그 커밋 뒤에 이슈를 닫는다.

## 쓰지 않는 것

- **`/wayfinder`** — 다중 세션 규모를 결정 티켓 지도로 그리는 스킬. ⏳ **지금은 그 지도가 비어 있다** — 옛 지형은 2026-08-18 에 걷어냈고 새 지형은 [순서표](../plan/02-order.md)가 졌다(2026-09-06)([처분표](../plan/disposal-map.md)). **그때도 실행하지 않는다** — 지도를 만드는 것은 회차의 일이지 명령의 일이 아니다.
