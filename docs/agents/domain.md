...
- **`docs/adr/`** — 착수할 영역에 걸리는 ADR. ⚠ **수를 여기 안 적는다** — 기능이
  끝날 때마다 늘고, 실제로 이 문서가 「스물셋」으로 두 번 낡았다(지금 `0001`~`0026`).
  재는 방법은 `ls docs/adr/00*.md` 다. 무엇이 왜 발행됐는지는 각 ADR 의 머리에 있고,
  발행 규칙은 **기능 종료 시점**이다(`/round` §9).
...
| 실행 순서와 상태 | GitHub Issues — **상태의 단일 진실**. `./scripts/frontier.sh` 가 지금 값을 산출한다 |
| 용어의 정의 | `CONTEXT.md` |
