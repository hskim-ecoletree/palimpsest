...
**판정: 통과 — 지목 가능률 90% · 저작 단가 중앙값 8.2분/건(n=5). 다만 출처가 6건에서 말랐다**
...
**8.2분/건은 비싸지 않다.** G4가 무너진다면 그 이유는 시간이 아니다. 그런데 저작은 멈췄다.
...
| 이번 실측이 말하는 것 | **저작한다 — 8.2분/건에. 다만 제품 문서가 관계를 담고 있는 동안만.** |
