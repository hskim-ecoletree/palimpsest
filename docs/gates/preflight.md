...
**38건 중 31건이 같은 답이고, 판정 근거가 몇 문장으로 수렴했다.**
D 블록부터 답이 `"인가 관련 코드이지만 주석이라 동작하는 코드가 아님"` 한 줄로 굳었고,
그 뒤로 건당 시간이 한 자리 초로 내려앉았다.

**이것은 판정자가 빨라진 것이 아니라 과제가 판정이 아니게 된 것이다.**
층 ①·③에서 등간격으로 뽑은 표본이 주석·비인가 코드에 압도적으로 쏠려,
`▶` 줄이 주석인지만 보면 답이 나오는 계열이 되었다.

**T2의 22.9초와 나란히 놓을 수 없다.** 과제 문구는 같았지만 후보의 성질이 달랐다 —
T2는 `아니오` 21 · `가드(인가)` 7 · `가드(인증)` 5로 세 종류가 섞여 있었고
[종류별 단가가 19.3 / 30.3 / 43.5초로 갈렸다](#t2--라벨-판정-30건).
T3의 표본에는 비싼 종류가 거의 없다.

> **이것이 이 회차의 진짜 산출이다.** 절감 장치를 재려면 먼저 **판정이 실제로 일어나는 표본**이
> 있어야 한다. T2의 제외 규칙(고유 30건 제외)이 비싼 계열을 함께 걷어냈다.
...
1. **표본이 판정을 요구해야 한다.** `아니오` 31/38은 과제가 아니다.
   T2가 보인 종류별 단가(19.3 / 30.3 / 43.5초)를 층으로 삼아 **비싼 계열을 섞어 뽑는다.**
2. **드리프트를 블록 안으로 접는다.** A-블록을 앞뒤 두 번이 아니라 **블록마다 끼워 넣거나**,
   장치를 블록이 아니라 건 단위로 교차시킨다(순서 고정을 포기하고 교차 배치를 사전 등록).
3. **판정자가 워밍업을 마친 뒤 시작한다.** A1의 4건(223.5 · 26.8 · 143.0 · 159.9초)은
   과제 형식을 다시 익히는 시간이다. 이 회차는 그것을 기준선으로 삼았다.
