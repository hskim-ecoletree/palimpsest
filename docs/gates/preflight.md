...
| 지목 가능률이 기록된다 | **충족** — 아래 |
| 저작에 걸린 시간이 기록된다 (판단 성분만) | **아직 못 쟀다 — 측정 대기.** 개념 20개를 저작한 것은 에이전트이고 사람이 저작한 적이 없어 잴 시간이 존재하지 않았다. [소유자 판단 §2](../instructions/2026-08-11-owner-direction.md#2--t6-저작-시간-t2와-같은-자리에서-두-수를-접지-않는다): **T2와 같은 세션에서 표본 8건으로 재되 두 수를 접지 않는다** → [`t6-authoring-sheet.md`](../../corpus/tasks/t6-authoring-sheet.md) |
