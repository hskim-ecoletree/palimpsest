...
**판정: 통과 — 과제 7건·대조군 5개·규모 코퍼스 확정. 다만 등록한 것은 형태이지 숫자가 아니다**
...
| 기준 | 결과 |
|---|---|
| 과제 5건 이상이 각각 질문·기대 산출의 형태·채점 절차를 갖고 커밋된다 | **충족** — **7건**(E1~E7). 셋 다 결손 0 |
...
복제가 아니므로 [R-24](../plan/00-risks.md#r-24)가 경고한 **캐시 적중률·이름 충돌 분포
왜곡이 없다.** 다섯 저장소 전부 실제 구현 코드다. 다만 합산 코퍼스는 저장소 경계가
있어 단일 저장소와 캐시 적중 패턴이 다르므로, E7이 **적중률을 함께 보고하도록** 절차에 넣었다.
