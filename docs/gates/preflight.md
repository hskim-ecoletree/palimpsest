...
T10의 인수 기준은 `outcomes.toml` 기록 하나였고 이 처분은 그 밖이다.
[T9](#t9--조달-가능성-실측) 때와 같은 자리다 — **소유자 지시를 기다리는 빚으로 남긴다.**
빚이라는 사실을 여기 적는 것이 조용히 넘어가지 않는 방법이다.

`Defect` 쪽은 조건절(*"소급 결박할 수 없으면"*)이 발동하지 않았다. 측정이 R-27 §대응3의
*"`Defect`는 올라탈 곳이 있다"* 를 확인했으므로, 처분을 적용할 때 그 절은 빼야 한다.
