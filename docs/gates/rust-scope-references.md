...
| **「아직 없다」** | **19**(`target/` 제외) | 열여덟은 다른 기능(F04·F07·F09·F20 …)의 것이라 이 회차와 무관하다. 이 회차가 낡게 만든 자리는 **하나** — `corpus/criteria.toml:874` 의 *"파일 간 해소는커녕 파일 내 참조도 아직 없다"*. 그 문장은 S0 추출기에 대한 것이라 지우지 않고 **날짜 붙은 정정을 덧붙였다** |
