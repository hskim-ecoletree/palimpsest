...
> ## ⛔ 아직 채우지 마라 — 2026-08-11 (갱신)
>
> **이 표는 은퇴한 표집(v1) 위에 서 있다. 재표집이 진행 중이다.**
>
> **정정 — 앞선 결함 보고 ①은 틀렸다.** "고유 좌표가 28개"라고 적었으나
> 전체 경로로 세면 **30개가 맞다**. 파일명만으로 중복 제거를 해서 별개 좌표를 접었다.
>
> **진짜 결함은 더 아래에 있었다.** `portal-backend-aa-task/` 는 별도 모듈이 아니라
> `portal-backend` 의 **git worktree**(브랜치 `feat/aa-task`)인데 `worktrees/` 밖에 놓여
> 등록된 범위 규칙(`worktrees/ 제외`)을 빠져나갔다. 그 결과 **풀 1,036 중 289건(27.9%)이
> 바이트 단위로 동일한 클론**이었다. 문제의 두 쌍도 내용이 완전히 같아 판정자에게는
> 구분되지 않는다 — 좌표는 둘인데 실물 코드는 하나다.
>
> **재질문율은 이 표로 잴 수 없다.** 심은 중복의 자리가 소유자에게 공개됐다
> (결함 보고 과정에서 났다). 어디가 중복인지 아는 상태의 판정은 재질문율이 아니다.
>
> **소요 시간 측정은 여전히 유효하다** — 그쪽은 블라인드를 요구하지 않는다.
>
> **처분: 소유자 판단 A — worktree 제외, 전면 재표집.**
> 기록: [`docs/instructions/2026-08-11-owner-direction.md`](../../docs/instructions/2026-08-11-owner-direction.md) §1
>
> 재표집이 끝나면 이 표는 통째로 교체된다.
