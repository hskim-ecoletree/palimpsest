...
프로젝트의 결정 기록(ADR 등)과 계획을 코드 좌표에 결박해 두고, `pal touch <심볼>` 한 번으로
...
- **지원 언어** — 언어마다 되는 것이 다릅니다. `pal ledger` 가 언어별 등급을 보여 줍니다.
  ...
  - **Kotlin** — **파일 최상위 선언**(클래스 · `object` · 인터페이스 · 최상위 함수 · 최상위 프로퍼티)의 좌표와 결정 결박까지입니다.
    메서드 · 멤버 프로퍼티 · `companion`/`object` 안의 멤버에는 좌표가 없고, 호출자와 참조 해소도 아직 없어 `호출자 0` 이 「아무도 안 부른다」는 뜻이 아닙니다.
