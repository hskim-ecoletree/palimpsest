...
| **고친다** | **3 파일** (자리 넷) | `f12-verify.py:477` — **조건 없는 `skip`**. 메시지가 지금 이미 거짓인데 조건문이 없어 아무도 못 잡는다 · `f12-verify.py:525` — 삭제된 `features/F12-plan-binding.md` 를 열어 **「못 읽었다 → 대조 불가」로 조용히** 떨어진다. ★ **독립 리뷰가 잡았다** — 내 조사가 「남긴다 39 = 접점 0」으로 셌는데 **접점이 있었다** · `f06-verify.py:246` 과 `f11-verify.py:472` 도 **실제로 고쳤다** — 「삭제가 스스로 고친다」로 적어 놓고 사유(「아직 없다」 → 「영영 없다」)를 손으로 갈랐다. ★ **독립 리뷰 2 라운드가 잡았다** — 판정표가 실물과 갈려 있었다 |
| **남긴다** | **37** | 접점 0 · 변경 0 |
