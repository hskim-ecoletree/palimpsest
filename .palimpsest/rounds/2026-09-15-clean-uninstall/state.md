...
종료 보고는 [`report.md`](report.md) 다. 완수 조건 22 전부 통과 · 발견 원장 열림 0 ·
독립 리뷰 상한 2 소진 · 결박 다섯 · 종료 기준선 `baseline/03-baseline-end.md`.
판정 근거는 push 한 마지막 SHA 의 런이다 — 그 뒤 push 가 더 있으면 **그 런으로 다시 댄다**.
...
## 독립 리뷰 R2 — 2026-09-17 (상한 2 소진)

원 반환문 `review/r2-raw.md`. 발견 14(유효 8 · 기각 6) — 금지역 2 · 거짓신호 2 · 미관 10.
합격선 축은 **통과 21 · 미측정 1**: E1 하나가 갈렸다.

- **금지역(E1 전사)** — 조건 문면은 「**push 한 마지막 SHA** 의 런」인데, 전사는 그 **부모**(`46d4e03`)의 런
  `35120132604` 를 근거로 통과를 적었다. R1 이 잡은 자리가 한 칸 위로 옮겨간 것이다.
  전사 커밋 `65d6c02` 의 런이 끝나 세 OS 초록이고 `e1-count.py` 가 그 런에서 어긋남 0 을 내면 그때 선다.
- **거짓신호 둘** — B5 의 CI 쪽 방은 착수 바이너리가 설치한 방이 아니라 **매니페스트만 되돌린 모사**다(문면의 방은
  `oracle/T2-b5-start-binary.txt` 한 판) · `e1-count.py` 가 **끝나지 않은 런**에서 계약된 `rc=2` 가 아니라 미포착 예외로 죽는다.
- **자기 산출 둘** — 종료 보고 부재 · 이 `state.md` 의 단계 표기가 실물보다 뒤(지금 고쳤다).
- 기각 여섯 중 하나는 워킹트리의 `doctor.rs` 변이를 회차 잔재로 의심한 것인데, **그때 돌고 있던 다른 시험의 변이**였다(`ps aux` 로 확인).
