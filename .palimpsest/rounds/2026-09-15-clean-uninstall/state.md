...
## 지금 단계 — **승인 요청 앞** (조건 설계 평가 상한 소진 · 사전부검 R1 레코드 바로잡기 진행 중)
...
| 조건 설계 평가 | R1 **닫혔다** — 걸림 15 · 기각 8 · 원 반환문 `conditions-audit/r1-raw.md` · 처리 방침은 `intent.md` `## 개정`(조건 20 · 허용 목록 `L`). R2 **닫혔다** — 걸림 13(실패 6 을 고침) · 기각 6 · `conditions-audit/r2-raw.md` · 조건 21 · 스냅샷의 정의 · 상한 소진 |
| 착수 전 기준선 | 진행 중 — `baseline/00-baseline-start.md` |
| 승인 | 안 받았다 — 올릴 때 소유자 판정 둘을 함께 묻는다: B6(사용자가 고친 pal 파일을 지운다) · D4(worktree 가 있으면 기본 uninstall 도 밖을 안 건드린다) |
