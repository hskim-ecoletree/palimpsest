...
## 지금 단계 — **판정 전사 · 게이트가 섰다. 독립 리뷰와 종료 걸음이 남았다**

| 단계 | 상태 |
|---|---|
| 착수 ~ 승인 · 구현 · 효과 | **닫혔다** — 조건 29 |
| 판 `p1-binary-nul` | **닫혔다** — 확대(NUL 규칙) · 실행 `ba0a983` · 측정 `oracle/p1-X/` · 레코드 DL1 13 |
| 판 `e3-effect` | **닫혔다** — E3 대조 불가 · 봉인 기준선 결함 · 레코드 DL2 19 |
| 판 `p2-caller-sites` | **닫혔다** — 정정(E2 ⑵ · E4 호출자 집합) · 개정 `403d589` · 재측정 `eb9c936` · 이슈 `#156` · 레코드 DL3 23 |
| 종료 기준선 | **돌았다** — `baseline/runner-post.md`. 이 회차가 만든 새 어긋남 일곱(f06 ③ 둘 · f09 ① 다섯)을 스크립트에서 고쳤다(작업 트리 · 재실행 중) |
| **G2** | **반증** — `scripts/f06-verify.py` 가 원본 ditto 에 캐시 2,451 개를 썼다 · 소유자 결정으로 더해진 파일만 지움 · `oracle/G2-violation.txt` · `MS2-09` |
| 결박 | **닫혔다** — `66108c5` · `pal doctor` 위반 0 |
| 판정 전사 · 게이트 | **닫혔다** — `792a4da` · 통과 24 · 반증 1(G2) · 대조불가 1(E3) · 미측정 3(A6 · F1 · F2) |
| 독립 리뷰 | **R1 진행 중** — 상한 4 |
| 남은 것 | 스크립트 수정 커밋(f06 캐시 격리 · f06 ③ 대조값 · f09 ① 표본) · `report.md`(초안 스크래치 `close/report-draft.md`) · `#135` 코멘트(초안 `close/comment-135.md`) · 최종 HEAD `cargo xtask test`(A6 ⑴) · push 한 번 · F2 |
