...
빌드된 `pal`을 시험 harness가 아닌 새 임시 Git 저장소에서 실행했다. approval 저장소도 그
저장소 밖의 별도 임시 디렉터리였다. 절대 경로, 사용자별 값, approval digest는 공유 산출물에
보존하지 않았다.
...
없었다. 입력 원본은 [`template/`](template/)에 있다.
