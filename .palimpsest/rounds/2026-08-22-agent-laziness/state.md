...
| **전사** | 죽어 있던 상자를 켰다 — 판정 내용은 안 바뀌었다 (세는 자리: `record.py conditions`) |
...
13. ★★ **격리 사본에서 음성 대조를 태울 때는 그 안에서 `xtask` 를 다시 빌드해라.**
    `repo_root()` 가 `env!("CARGO_MANIFEST_DIR")` 라 **컴파일 시점에 원본 경로가 박힌다** —
    사본을 복사만 하고 원본의 바이너리로 돌리면 **사본의 흠집을 안 보고 원본을 재서
    초록이 난다.** 독립 리뷰가 실제로 그 초록을 한 번 받았다.
    확인: `strings target/debug/xtask | grep <사본경로>` 가 비면 원본을 재고 있다.
14. ★ **`target/` 의 바이너리가 낡으면 `버전에_커밋이_실려_있다` 가 거짓 실패한다.**
...
[#94] `게이트파서` 선언이 코드 갈래를 못 따라간다 · [#95] `K9` 는 쓸 때 아직 참이 아닌 문장으로만 닫힌다.
