...
def _infer_column_type(values: List[str]) -> tuple:
    ...
    Rules:
    - All dates (YYYY-MM-DD) → "date" with date values
    ...
    for val in values:
        if val == "":
            ...
            # Try date first
            if _is_date(val):
                converted.append(_parse_date(val))
                has_date = True
            # Try bool
    ...
    if has_date:
        # If we have dates and anything else, convert to string
        if has_non_numeric or has_bool or has_float or any(isinstance(v, int) and v is not None and not isinstance(v, bool) for v in converted):
            final_type = "str"
            final_values = []
    ...
    elif has_non_numeric or (has_bool and (has_float or any(isinstance(v, int) and v is not None for v in converted if not isinstance(v, bool)))):
    ...
    for row in table["rows"]:
        ...
        for col_idx, col in enumerate(table["columns"]):
            ...
            if isinstance(val, date):
                row_dict[col["name"]] = val.strftime("%Y-%m-%d")
            else:
...
def missing_report(table: dict) -> dict:
    ...
    Returns:
        Dictionary mapping column names to missing ratios (0.0 to 1.0)
    """
    report = {}
    num_rows = len(table["rows"])

    if num_rows == 0:
        # If no rows, ratio is 0.0 for all columns
        for col in table["columns"]:
            report[col["name"]] = 0.0
    else:
        for col_idx, col in enumerate(table["columns"]):
            missing_count = sum(1 for row in table["rows"] if row[col_idx] is None)
            ratio = missing_count / num_rows
            report[col["name"]] = ratio
...
def pivot_count(table: dict, row: str, col: str) -> dict:
    ...
    Args:
        table: Input table
        row: Column name for row grouping
        col: Column name for column grouping

    Returns:
        Dictionary: {row_value: {col_value: count}}

    Raises:
        NormError: If row or col column not found
    """
    row_idx = None
    ...
    if row_idx is None:
        raise NormError(f"Column '{row}' not found")
    ...
    if col_idx is None:
        raise NormError(f"Column '{col}' not found")

    # Build pivot table
    pivot = {}
    for table_row in table["rows"]:
        row_val = table_row[row_idx]
        col_val = table_row[col_idx]

        if row_val not in pivot:
            pivot[row_val] = {}

        if col_val not in pivot[row_val]:
            pivot[row_val][col_val] = 0

        pivot[row_val][col_val] += 1
