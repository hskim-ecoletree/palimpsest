    ...
    for row in table["rows"]:
        ...
        for v in row:
            if v is None:
                ...
                row_strs.append(_escape_csv_field(v.strftime("%Y-%m-%d"), sep))
            else:
...
def missing_report(table: dict) -> dict:
    ...
    if num_rows == 0:
        ...
        for col_idx, col in enumerate(table["columns"]):
            ...
            result[col["name"]] = missing_count / num_rows
...
def pivot_count(table: dict, row: str, col: str) -> dict:
    ...
    if row not in col_names:
        raise NormError(f"없는 열이다: {row}")
    if col not in col_names:
        raise NormError(f"없는 열이다: {col}")
