...
def _is_table_separator(line: str) -> bool:
    ...
    # Each cell should match pattern: optional :, dashes, optional :
    for cell in cells:
...
def _parse_table_separator(line: str) -> list[str]:
    """Parse table separator line and return alignment for each column.
    Returns list of 'left', 'center', 'right', or ''."""
    line = line.strip()
    cells = line.split("|")
    cells = [cell.strip() for cell in cells if cell.strip()]
...
def _parse_table_row(line: str) -> list[str]:
    """Parse a table row and return list of cells."""
    line = line.strip()
    if line.startswith("|"):
        line = line[1:]
    if line.endswith("|"):
    ...
    while i < len(lines):
        ...
        if line.startswith("#"):
            ...
            if level_match:
                ...
                if level > 6:
                    # Headings only go up to H6, so treat as paragraph
                    para_lines = [line]
                    i += 1
        ...
        # Table detection: check if current line contains | and next line is separator
        if "|" in line and i + 1 < len(lines) and _is_table_separator(lines[i + 1]):
            # Parse header row
            header_cells = _parse_table_row(line)
            aligns = _parse_table_separator(lines[i + 1])
            ...
            while i < len(lines):
                ...
                if _is_table_separator(current_line):
                    break

                # Check if this line contains the start of another block type
                if current_line.startswith("#") or current_line.startswith(">") or _detect_list_type(current_line)[0] or re.match(r"^(---|___|\*\*\*)$", current_line.strip()):
                    break

                body_rows.append(_parse_table_row(current_line))
                i += 1
            ...
            for cell in header_cells:
                ...
                table_html.append(f"<th>{rendered_cell}</th>")
            ...
            for row_cells in body_rows:
                ...
                for idx, cell in enumerate(row_cells):
                    ...
                    if align:
                        table_html.append(f'<td style="text-align:{align}">{rendered_cell}</td>')
                    else:
                        table_html.append(f"<td>{rendered_cell}</td>")
                table_html.append("</tr>")
                ...
                while i < len(items):
                    ...
                    if indent == level and item_type == current_type:
                        ...
                        if nested:
                            ...
                            html.append(f"<li>{_render_inline(text)}</li>")
